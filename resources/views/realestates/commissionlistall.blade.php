...
@section('css')
	...
	#select2-selbank-results {font-family: 'Noto Sans Khmer', sans-serif;font-size:16px;}
       ...
       #divfooter{
        ...
        .tableFixHead thead th { position: sticky; top: 0; z-index: 1;background-color:rgb(221, 233, 233) }

        .tableFixHead2{ overflow: auto;border:1px dotted black;}
        .tableFixHead1{ overflow: auto;border:1px dotted black;margin-top:5px;}
        .tableFixHead1 thead th { position: sticky; top: 0; z-index: 1;background-color:rgb(221, 233, 233) }
        .tbl_checkamt td{
          word-wrap: break-word;
...
@section('content')
    ...
    <div class="" style="margin:-15px 0px 10px -10px;">
          <table id="tbl0" class="tbl0" style="margin:0px;padding:0px;">
              <tr>
                  ...
                  <td class="kh16-b" style="width:160px;padding-left:5px;display:none;">
                  ...
                  <td class="kh16-b" style="width:160px;display:none;">
                      <div class="input-group" style="width:160px;">
                ...
                <td class="kh16-b" style="width:120px;border-style:none;padding:0px;">
                    <select name="selmonth" id="selmonth" class="form-select" style="width:120px;">
                        <option value=""></option>
                        @for($n=1;$n<=12;$n++)
                            <option value="{{ $n }}" @if($month==$n) selected @endif>{{ dokhmermonth($n) }}</option>
                        @endfor
                    </select>
                </td>
                ...
                <td class="kh16-b" style="width:120px;padding:0px;border-style:none;">
                    <select name="selyear" id="selyear" class="form-select" style="width:100px;">
                      @for($i=2020;$i<2225;$i++)
                  ...
                  <td style="width:150px;padding-left:10px;">
                      <select class="kh16-b" name="sel_data" id="sel_data" style="width:150px;">
                          ...
                          <option value="-1">មិនទាន់ទូទាត់+ខែចាស់</option>
                  ...
                  <td style="padding:0px 10px;width:200px;">
                      ...
                      <button id="btnpreview" class="mybtn kh12-b" onclick="printSelectedColumns()">Preview</button>
    ...
    <div id="" class="row" style="margin-top:-10px;">
        ...
        <div class="table-responsive" style="padding:0px; margin:5px 0px;">
            <table class="">
                <tr>
    ...
    <script type="text/javascript">
        ...
        if (!isAdmin) {
         ...
         function printSelectedColumns() {
            ...
            var d1=$('#d1').val();
            var d2=$('#d2').val();
            var m=$('#selmonth option:selected').text();
            var y=$('#selyear option:selected').text();
            ...
            let displayText = selectedLabels.length > 5
                    ...
                    : selectedLabels.join(', ');
            let data_text=$('#sel_data option:selected').text();
            $('#rpt_title').text('កម្រៃជើងសារ '+ data_text + ' ខែ '+ m + 'ឆ្នាំ ' + y);
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','#btnprintall',function(e){
                ...
                var redirectWindow = window.open('{{ url('/') }}'+'/realestate/getcommissionlistall/print?saler='+saler+'&d1='+d1+'&d2='+d2+
                '&isprint='+1+'&salername='+salername+'&location_id='+8+'&pid='+pid+'&seldata='+seldata+'&alldate='+alldate+'&selgroup='+selgroup , '_blank');
        ...
        function calculateSum() {
            ...
            $('input[name="ckid"]:checked').each(function() {
                ...
                let inputEl = row.find('td:eq(8) input');
                let amount = parseFloat(inputEl.val().replace(/,/g, '')) || 0;
                let comleftText = row.find('td:eq(11)').text().trim();
        ...
        // });//
        // $(document).on('click', '#btnsortproperty', function (e) {//this sort affect total row
        //     e.preventDefault();
        //     const table = document.getElementById('tbl_commission_list');
        //     if (!table) return;
        ...
        $(document).on('click', '#btnsortproperty', function (e) {
            ...
            // ✅ Separate total row (last row)
            const totalRow = rows.pop(); // remove last row
            // rows now contains sortable rows only
            ...
            // --- Sort rows by column 2 ---
            rows.sort((rowA, rowB) => {
                const valA = $(rowA).find('td').eq(2).text().trim();
                const valB = $(rowB).find('td').eq(2).text().trim();
            ...
            // --- Reinsert sorted rows ---
            rows.forEach((row, index) => {
                $(row).find('td').eq(0).text(index + 1); // reset No column
                tbody.appendChild(row);
            });

            // ✅ Append total row back at the end
      ...
      function printdeposit(id,groupid,rpttitle){
        ...
        function getcomissionlist()
        {
            ...
            $.ajax({
                ...
                type: 'GET',
                url: url,
                data: {alldate:alldate,d1:d1,d2:d2,location_id:location_id,saler:saler,selgroup:selgroup,pid:pid,seldata:seldata,m:m,y:y},
                ...
                success: function (data) {
                    ...
                    refreshtableFixHead(215);
                    document.querySelectorAll('#tbl_commission_list th').forEach(header => {
                    ...
                    // $('#rpt_title').text('កម្រៃជើងសារ គិតពី '+ d1 + 'ដល់ ' + d2);
                    // $('#rpt_title1').text('ប្លុក ' + displayText);
                    $('.txtcommission').toArray().forEach(function(field){
                        new Cleave(field, {
                            numeral: true,
