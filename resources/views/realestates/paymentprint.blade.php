                ...
                <tbody>
                   ...
                   @foreach ($transfergroup as $key => $g)
                        @php
                            ...
                            $total +=$g->amount+$g->cuscharge+$g->bonus;
                            $nextpayment=$g->nextpayment;
                        @endphp
                       <tr>
                        <td class="kh14-b" style="text-align:center;">{{ ++$key }}</td>
                        <td class="kh14-b">{{ date('d-m-Y',strtotime($g->dd))  }}</td>
                        <td class="kh14-b">{{ date('d-m-Y',strtotime($g->payformonth))  }}</td>
                        <td class="kh14-b">{{ $g->deposit_via??'' }}</td>
                        <td class="kh14-b" style="text-align:right;">{{ phpformatnumber($g->amount+$g->bonus) . ' ' . $g->currency->shortcut }}</td>
                        <td class="kh14-b" style="text-align:right;">{{ phpformatnumber($g->cuscharge) . ' ' . $g->currency->shortcut }}</td>
                        <td class="kh14-b" style="text-align:right;">{{ phpformatnumber($g->amount+$g->cuscharge+$g->bonus) . ' ' . $g->currency->shortcut }}</td>

                       </tr>
                   @endforeach
                       <tr>
                            <td class="kh22-b" style="text-align:right;" colspan=6>
                                <span style="float:left;">បង់បន្ទាប់ : {{ date('d-m-Y',strtotime($nextpayment)) }}</span>
                                សរុបទឹកប្រាក់
                            </td>
                       ...
                       @if($discount<>0)
                        <tr>
                                ...
                                <td class="kh22-b" style="text-align:right;">
                                {{ phpformatnumber(abs($discount))  . ' ' . $cur }}
                                </td>
                        </tr>
                       @endif
                       @if($transfer->nopunish==1)
                            <tr>
                                <td class="kh22-b" style="text-align:right;" colspan=6>
                                    លើកលែងការពិន័យ
                                </td>
                                <td class="kh22-b" style="text-align:right;">
                                    {{ phpformatnumber(abs($transfer->discount_amount))  . ' ' . $cur }}
                                </td>
                            </tr>
                            <tr>
                                <td class="kh22-b" style="text-align:right;" colspan=6>
                                ...
                                <td class="kh22-b" style="text-align:right;">
                                    {{ phpformatnumber(abs($total) - abs($transfer->discount_amount))  . ' ' . $cur }}
                                </td>
                            </tr>
                        @endif
                        @if($transfer->bank_amount && $transfer->bank_amount<>0)
                            <tr>
                                <td class="kh22-b" style="text-align:right;" colspan=6>

                                    {{$transfer->deposit_via}}
                                </td>
                                <td class="kh22-b" style="text-align:right;">
                                {{ phpformatnumber(abs($transfer->bank_amount))  . ' ' . $cur }}
                                </td>
                            </tr>
                       @endif
                      @if($transfer->cash_amount && $transfer->cash_amount<>0)
                            <tr>
                                <td class="kh22-b" style="text-align:right;" colspan=6>
                                    ទូទាត់ជាសាច់ប្រាក់
                                </td>
                                <td class="kh22-b" style="text-align:right;">
                                {{ phpformatnumber(abs($transfer->cash_amount))  . ' ' . $cur }}
                                </td>
                            </tr>
                       @endif
                        @if($transfer->cuscharge_debt && $transfer->cuscharge_debt<>0)
                             <tr>
                                <td class="kh22-b" style="text-align:right;" colspan=6>
                                    ទឹកប្រាក់ជំពាក់
                                </td>
                                <td class="kh22-b" style="text-align:right;">
                                    {{ phpformatnumber($transfer->cuscharge_debt)  . ' ' . $cur }}
                                </td>
                            </tr>
                        @endif
                       {{-- <tr>
                            <td colspan=7 style="padding:0px;border-style:none;">
                                <table id="tblsummary" class="" style="padding:0px;margin:0px;">
                        ...
                        @if($transfer->note)
                            <tr>
                                <td class="kh16-b" style="color:red;text-align:center;" colspan=7>
                                    {{ $transfer->note }}
                                </td>
                            </tr>
                        @endif
