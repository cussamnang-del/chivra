                                            ...
                                            @foreach (App\PartnerTransfer::showbygroup($tr['id'],$tr['groupid']) as $item)
                                                ...
                                                <tr class="kh12-b" style="">
                                                    <td style="text-align:center;">{{ $item->id }}
                                                         @if(Auth::user()->role->name=='Admin')
                                                            @if(($item->trancode==1 || $item->trancode==4) && $item->sendername )

                                                                <br>
                                                                <a href="" class="mybtn btnaddcommission" data-id="{{$item->id}}" data-group_id="{{$item->ref_group_id}}" style="color:purple;border-style:none;">Add Coms</a>
                                                    ...
                                                    <td>{{ $item->note }}</td>
                                                    @if(Auth::user()->role->name=='Admin')
                                                        @if($item->amount==0 && $item->trancode==-1)
                                                            <td style="text-align:center;">
                                                                <a class="kh16-b btndel_com0" style="color:red;" href="#" data-id="{{ $item->id }}"><i class="fa fa-trash"></i></a>
                                                            </td>
                                                        @endif
                                                    @endif
                                                </tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      function printdeposit(id,groupid,rpttitle,propertyname){
        ...
        $(document).on('click','.btndel_com0',function(e){
            e.preventDefault();
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        dataType:'JSON',
                        contentType: 'application/json;charset=utf-8',
                        url: "{{ route('realestate.deletecomzero') }}",
                        data: { id:id},
                        success: function (data) {
                            //console.log(data);
                            if (data.success === true) {
