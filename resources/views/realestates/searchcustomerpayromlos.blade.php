    ...
    @foreach ($transfers as $k => $tr)
        ...
        <tr class="@if($bal==0) @elseif(floatval($tr->qtyleftday)<=0 && floatval($tr->qtyleftday+10)>=0) c_orange @elseif(floatval($tr->qtyleftday)<0 && floatval($tr->qtyleftday+10)<0) c_red @endif">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        @if (Auth::user()->role->name<>'Admin')
                            ...
                            @else
                                <li class="li_code131">
                                    <a class="dropdown-item kh16-b" href="{{ route('realestate.showromloslist',['id'=>$tr->main_id,'property_group'=>$tr->main_property_group,'property_id'=>$tr->main_property_id,'customer_id'=>$tr->main_parrent_id,'customertype'=>$tr->main_customertype,'term'=>$tr->main_term,'rate'=>$tr->main_interest_rate,'startdate'=>$tr->main_startdate,'enddate'=>$tr->main_enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->main_payinmonth,'ispayromlos'=>1,'sendername'=>$tr->main_property,'qtyleftday'=>$tr->qtyleftday]) }}" target="_blank"><i class="fa fa-building"></i> បង់រំលស់</a>
                                </li>
                                ...
                                <li class="li_code133">
                                    <a class="dropdown-item kh16-b" href="{{ route('realestate.printromloslistforcustomer',['id'=>$tr->main_id,'customer_id'=>$tr->main_parrent_id,'customertype'=>$tr->main_customertype,'term'=>$tr->main_term,'rate'=>$tr->main_interest_rate,'startdate'=>$tr->main_startdate,'enddate'=>$tr->main_enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->main_payinmonth,'ispayromlos'=>1,'sendername'=>$tr->main_property,'qtyleftday'=>$tr->qtyleftday]) }}" target="_blank"><i class="fa fa-print"></i> ព្រីនតារាងបង់ប្រាក់</a>

                                </li>
                                 <li class="li_code1314">
            ...
            <td class="kh16-b" style="text-align:right;">
                <a href="{{ route('realestate.showdeposit',['id'=>$tr->main_id,'customer_id'=>$tr->main_parrent_id,'customertype'=>$tr->main_customertype,'term'=>$tr->main_term,'rate'=>$tr->main_interest_rate,'startdate'=>$tr->main_startdate,'enddate'=>$tr->main_enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->main_payinmonth,'sendername'=>$tr->main_property]) }}" class="kh16-b " target="_blank" style="margin:0px;padding:2px;@if($bal==0) @elseif(floatval($tr->qtyleftday)<=0 && floatval($tr->qtyleftday+10)>=0) color:black; @elseif(floatval($tr->qtyleftday)<0 && floatval($tr->qtyleftday+10)<0) color:white; @endif">{{ phpformatnumber(abs($tr->deposited)) .$tr->currency->sk }}</a>
            </td>
            ...
            <td class="kh16" style="text-align:right;">{{ phpformatnumber($tr->main_payinmonth??'0') . $tr->currency->sk}}</td>
