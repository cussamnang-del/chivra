...
<script type="text/javascript">
    ...
    if (!isAdmin) {
        ...
        if (!userPerms.has('1.3.11')) {
            $('#old_cuscharge_debt').attr('readonly',true);
        }
        if (!userPerms.has('1.3.12')) {
    ...
    var deposit = new Cleave('#deposit', {
          numeral: true,
          numeralPositiveOnly: true,
          numeralThousandsGroupStyle: 'thousand'
      });
        var bonus = new Cleave('#bonus', {
          numeral: true,
          numeralPositiveOnly: true,
          numeralThousandsGroupStyle: 'thousand'
      });
      var cleaveoverprice = new Cleave('#overprice', {
        ...
        var cleave_oldebt = new Cleave('#old_cuscharge_debt', {
          numeral: true,
          numeralPositiveOnly: true,
          numeralThousandsGroupStyle: 'thousand'
      });
      ...
      function addMonths(numOfMonths, date = new Date()) {
        ...
        $(document).on('change','#overprice,#overday,#old_cuscharge_debt',function(e){
            e.preventDefault();
        ...
        $(document).on('change','#deposit,#bonus',function(e){
            e.preventDefault();
            var bal=$('#balance').val().replace(/,/g, '');
            var dep=$('#deposit').val().replace(/,/g, '');
            var bonus=$('#bonus').val().replace(/,/g, '');
            var bal1=parseFloat(bal)-parseFloat(dep)-parseFloat(bonus);
            var ovamt=$('#overamount').val().replace(/,/g, '');
