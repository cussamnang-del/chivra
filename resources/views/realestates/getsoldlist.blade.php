...
<div class="tableFixHead" style="padding:0px;">
    <table id="mytable" class="table table-bordered table-hover tbl_transferlist" style="table-layout:fixed;">
        <thead style="text-align:center;" class="kh16">
            ...
            <th style="width:150px;padding:0px;">
                <div class="dropdown" style="display:inline;">
                    <button type="button" class="mybtn dropdown-toggle kh16" data-bs-toggle="dropdown" style="width:100%;background-color:aqua;">
                        នៅខ្វះ
                    </button>
                     <ul class="dropdown-menu">
                        <li><a class="dropdown-item kh16 btnselbal" data-v="0" href="#">បង់រួចរាល់</a></li>
                        <li><a class="dropdown-item kh16 btnselbal" data-v="1" href="#">មិនទាន់រួចរាល់</a></li>
                        <li><a class="dropdown-item kh16 btnselbal" data-v="-1" href="#">ទាំងអស់</a></li>
                    </ul>
                </div>
            </th>
        ...
        <tbody id="body_transaction">
            ...
            @foreach ($transfers as $k => $tr)
                ...
                <tr>
                    ...
                    <td class="kh16" style="text-align:center;padding:0px;">
                        <div class="dropdown">
                            <button style="width:100%;" type="button" class="mybtn dropdown-toggle kh14-b" data-bs-toggle="dropdown">{{ sprintf("%04d",$tr->id) }}</button>
                            <ul class="dropdown-menu">
                                ...
                                @if($tr->trancode==-8)
                                    ...
                                    <li class="li_code138"><a class="dropdown-item kh16-b btnready" style="@if(abs($tr->amount)-abs($tr->deposited)==0) display:block @else display:none @endif" href="#" data-id="{{ $tr->id }}" data-balance="{{ abs($tr->amount)-abs($tr->deposited) }}"><i class="fa fa-check"></i> {{ $tr->loancomplete==1?'Loan not yet Complete':'ល្វែងនេះទូទាត់រួចរាល់' }}</a></li>
                    ...
                    <td class="kh16-b" style="text-align:right;">
                        <a href="{{ route('realestate.showdeposit',['id'=>$tr->id,'group_id'=>$tr->ref_group_id,'customer_id'=>$tr->parrent_id,'customertype'=>$tr->partner->customertype,'term'=>$tr->term,'rate'=>$tr->interest_rate,'startdate'=>$tr->startdate,'enddate'=>$tr->enddate,'curid'=>$tr->currency_id,'cursk'=>$tr->currency->sk,'curname'=>$tr->currency->shortcut,'payinmonth'=>$tr->payinmonth,'sendername'=>$tr->sendername]) }}" class="kh16-b" target="_blank" style="margin:0px;padding:2px;text-decoration:underline;">{{ phpformatnumber(abs($tr->deposited)) .$tr->currency->sk }}</a>
                    </td>
...
<div class="">
    <table id="tbl_summary" class="table kh22-b" style="margin:0px;padding:0px;">
