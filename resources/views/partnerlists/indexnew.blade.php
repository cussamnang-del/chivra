            ...
            <table id="tblmain" class="table">
                <tr>
                    {{-- <td class="kh16-b" style="width:50%;">
                        ...
                        <div class="form-check" style="display:inline">
                            <input class="form-check-input checkbox-large" type="checkbox" name="ckalldate" id="ckalldate" >
                            <label class="form-check-label kh16-b" for="ckalldate">
                                All Date
                            </label>
                        </div>
                    </td> --}}
                    <td class="kh16-b" style="width:50%;">
                        គិតពី
                        <div class="form-check d-inline-flex align-items-center ms-2" style="float:right;margin-right:20px;">
                            <input class="form-check-input checkbox-large me-1"
                                type="checkbox"
                                name="ckalldate"
                                id="ckalldate">
                            <label class="form-check-label kh16-b mt-10"
                                for="ckalldate">
                                All Date
                            </label>
                        </div>
                ...
                <tr>
                    <td colspan=2 style="padding-top:10px;">
                        ...
                        <label class="form-check-label kh16-b" style="margin-left:10px;">
                            <input class="form-check-input kh16-b" type="checkbox" name="ckrunbalance" id="ckrunbalance" style="margin-left:5px;"> ShowBalance
                        </label>
                ...
                <tr>
                    ...
                    <td colspan=2 style="padding:0px;">
                        <input type="text" class="kh16" id="tableSearch" style="width:100%;background-color:lightgreen;border-style:none;"  placeholder="Search all column with rang amount like: 1000-2000" title="Type what you khnow">
                    </td>
                </tr>
            </table>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        function SearchList(callback)
        {
            ...
            var seelist=document.querySelector('input[name="optlist"]:checked').value;
            var linkdetail = document.getElementById("cklinkdetail").checked;
            var runbalance = document.getElementById("ckrunbalance").checked;
            var usd_id = $("#selcur option").filter(function() {
                return $(this).text() === 'USD';
            }).val();
            var thb_id = $("#selcur option").filter(function() {
            ...
            $.ajax({
                ...
                type: 'GET',
                url: url,
                data: { d1:d1,d2:d2,partner:partner,oldlist:oldlist,alldate:alldate,partnername:partnername,searchtran:searchtran,isbooklist:2,usd_id:usd_id,thb_id:thb_id,khr_id:khr_id,vnd_id:vnd_id,showby:showby,seelist:seelist,linkdetail:linkdetail,thai_list:thai_list,runbalance:runbalance},
                complete: function () {},
    ...
    $("#tableSearch_old").on("keyup", function () {
        var value = $(this).val().replace(/,/g, "").toUpperCase(); // remove commas from input
        $("#tbl_partner_list tbody tr").each(function () {
            var $row = $(this);
            ...
            $row.find("td").each(function () {
                var text = $(this).text().replace(/,/g, ""); // remove commas from cell text
            ...
            // Force collapse row hidden during search
            var $collapseRow = $row.next(".collapse");
            $collapseRow.removeClass("show");
        });
    });
    // this table search with rang amount like 1000-6000
    $("#tableSearch").on("keyup", function () {
        let value = $(this).val().trim().replace(/,/g, "").toUpperCase();
        let isRange = value.includes("-");
        let min = null;
        let max = null;
        // 🔹 Detect range search: 100-300
        if (isRange) {
            let parts = value.split("-").map(v => v.trim());
            min = parseFloat(parts[0]);
            max = parseFloat(parts[1]);
            // Invalid range → fallback to text search
            if (isNaN(min) || isNaN(max)) {
                isRange = false;
            }
            // Swap if user types 300-100
            if (min > max) {
                [min, max] = [max, min];
            }
        }
        ...
        $("#tbl_partner_list tbody tr").each(function () {
            ...
            $row.find("td").each(function () {
                ...
                // 🔹 RANGE SEARCH
                if (isRange) {
                    // Remove everything except numbers & dot
                ...
                // 🔹 NORMAL TEXT SEARCH
                    ...
                    let searchText = text.replace(/,/g, "").toUpperCase();
            ...
            // Always hide collapse row during search
