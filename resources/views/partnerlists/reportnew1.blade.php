@php
    ...
    function phpformatbalance($num)
    {
        $num = floatval($num);
        $int = number_format(floor($num));  // Format the integer with thousand separator
        $dec = fmod($num, 1);               // Get decimal part

        // If decimal = .00 → return only integer
        if ($dec == 0) {
            return $int;
        }

        // Format decimal
        $dec2 = number_format($dec, 2, '.', ''); // ex: ".25"
        $dec2 = substr($dec2, 1);               // remove leading zero → ".25"

        return $int . ' ' . $dec2;  // Example: "254 .25"
    }
    ...
    <table id="tbl_partner_list" class="table table-bordered table-hover" style="table-layout:fixed;">
        ...
        <tbody id="bodytransfer">
            ...
            @foreach ($ptls_new as $key =>$l)
                ...
                @endphp
                <tr style="">
                    <td class="kh14" style="text-align:center;width:80px;">{{ ++$key }}</td>
                    <td class="kh14" style="text-align:center;width:80px;">
                            @if($l->tranid)
                                @if($linkdetail=='true')
                    ...
                    <td class="kh14" style="text-align:center;width:120px;">
                        @if($l->ref_group_id)
                            @if($linkdetail=='true')
                    ...
                    <td class="kh14" style="width:180px;">{{ date('d-m-Y',strtotime($l->trandate)) }} {{ $l->trantime }}</td>
                    <td class="kh14" style="">{{ $l->tranname }}</td>

                    <td style="text-align:right;width:150px;" class="kh14-b @if($l->usd>0) cred @else cblue @endif">
                        {{ $l->usd<>0?phpformatnumber(-1 * $l->usd) .'$':'' }}
                        @if($runbalance=='true' && $l->usd <> 0)
                            <br> <span style="font-size:12px;color:gray;">{{ phpformatbalance(-1 * $tusd)  . '$'}}</span>
                        @endif
                    </td>
                    <td style="text-align:right;width:150px;" class="kh14-b @if($l->khr>0) cred @else cblue @endif">
                        {{ $l->khr<>0?phpformatnumber(-1 * $l->khr) .'R':''}}
                        @if($runbalance=='true' && $l->khr <> 0)
                            <br> <span style="font-size:12px;color:grey">{{ phpformatnumber(-1 * $tkhr)  . 'R'}}</span>
                        @endif
                    </td>
                    <td style="text-align:right;width:150px;" class="kh14-b @if($l->thb>0) cred @else cblue @endif">
                        {{ $l->thb<>0?phpformatnumber(-1 * $l->thb) .'B':''}}
                        @if($runbalance=='true' && $l->thb <> 0)
                            <br> <span style="font-size:12px;color:grey">{{ phpformatnumber(-1 * $tthb)  . 'B'}}</span>
                        @endif
                    </td>
                    @if(config('helper.col_vnd') == 1)
                        <td style="text-align:right;width:150px;" class="kh14-b @if($l->vnd>0) cred @else cblue @endif">
                            {{ $l->vnd<>0?phpformatnumber(-1 * $l->vnd) .'D':''}}
                                @if($runbalance=='true' && $l->vnd <> 0)
                                <br> <span style="font-size:12px;color:grey">{{ phpformatnumber(-1 * $tvnd)  . 'D'}}</span>
                            @endif
                        </td>
                    @endif
                    <td class="kh14" style="">{{ $l->rectel }}</td>
                    <td class="kh14" style="">{{ $l->sendertel }}</td>
                        <td class="kh14" style="">
                        {{ $l->recordby }}
                    </td>
                    <td class="kh14" style="">
            ...
            <tr style="background-color:aqua">
                <td colspan=5 class="kh16-b" style="color:black;">សរុប</td>
                <td style="text-align:right;font-family:Arial, Helvetica, sans-serif;font-weight:bold;" class="kh16-b @if($tusd>=0) cred @else cblue @endif">{{ $tusd<>0?phpformatnumber(-1 * $tusd) . '$':'' }}</td>
                <td style="text-align:right;font-family:Arial, Helvetica, sans-serif;font-weight:bold;" class="kh16-b @if($tkhr>=0) cred @else cblue @endif">{{ $tkhr<>0?phpformatnumber(-1 * $tkhr) .'R':''}}</td>
                <td style="text-align:right;font-family:Arial, Helvetica, sans-serif;font-weight:bold;" class="kh16-b @if($tthb>=0) cred @else cblue @endif">{{ $tthb<>0?phpformatnumber(-1 * $tthb) .'B':''}}</td>
                <td style="text-align:right;font-family:Arial, Helvetica, sans-serif;font-weight:bold;" class="kh16-b @if($tvnd>=0) cred @else cblue @endif">{{ $tvnd<>0?phpformatnumber(-1 * $tvnd) .'D':''}}</td>
                <td colspan=4></td>
