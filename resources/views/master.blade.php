...
<head>
    ...
    <style>
        ...
        #tbl_useraccount_master td,
        #tbl_useraccount_master th {
            white-space: normal;
...
<body>
            ...
            <div class="row">
                        ...
                        <tr>
                            <td style="width:160px;">
                                <div class="input-group" style="width:160px;">
                                    <input type="text" name="showdate_master" id="showdate_master"
                                        class="form-control"
                                        style="width:110px;height:30px;background-color:silver;font-size:16px;font-weight:bold;">
                                    <span class="input-group-text"><i class="fa fa-calendar"></i></span>
                                </div>
                            </td>
                            <td style="">
                                <select class="kh16-b" name="selusermaster" id="selusermaster"
                                    style="width:100%;height:30px;background-color:aquamarine">

                                </select>
                            </td>
                            <td style="">
                                <button class="mybutton" id="btnrefreshinmaster"
                                    style="border:1px solid black;float:right;height:30px;">Refresh</button>
                            </td>
                        </tr>
                    </table>
                ...
                <div class="row" style="padding:0px;margin:0px;">
                    <div class="table-responsive" style="padding:0px;margin:0px;">
                        <table id="tbl_useraccount_master" class="table table-bordered table-hover"
    ...
    <script src="{{ config('helper.asset_path') }}/admin/assets/plugins/vectormap/jquery-jvectormap-world-mill-en.js">
    </script>
    ...
    <script
        src="{{ config('helper.asset_path') }}/admin/assets/plugins/bootstrap-material-datetimepicker/js/moment.min.js">
    </script>
    <script
        src="{{ config('helper.asset_path') }}/admin/assets/plugins/bootstrap-material-datetimepicker/js/bootstrap-material-datetimepicker.min.js">
    </script>
    ...
    <audio id="notifySound" preload="auto">
        <source src="{{ config('helper.asset_path') }}/sounds/notify.wav" type="audio/wav">
    </audio>
    <!-- Optional: visible button for UX to ensure Chrome unlocks audio -->
    {{-- <script>
        ...
        function unlockAudio() {
            if (!isAudioUnlocked) {
                ...
                sound.play().then(() => {
                    ...
                    console.log("Unlock blocked ❌", err);
                });
        ...
        if (window.Echo) {
            window.Echo.channel('gold-orders')
            .listen('.new-order', (e) => {
                ...
                Swal.fire({
                    ...
                    text: `User ${e.order.user_id} submitted ${e.order.amount}`,
                    timer: 10000,
    ...
    <!-- Place this inside <body> somewhere -->
    <script>
        const sound = document.getElementById("notifySound");
        const bellIcon = document.getElementById("iconbell");
        let isAudioUnlocked = false;
        ...
        //         sound.currentTime = 0;
        //         isAudioUnlocked = true;
        //         bellIcon.style.color = "green";
        //         console.log("Audio unlocked ✅");
        //         alert("Notification sound enabled!");
        //     }).catch(err => console.log("Audio still locked ❌", err));
        // });
        document.getElementById("enableSoundBtn").addEventListener("keydown", () => {
            sound.play().then(() => {
                sound.pause();
                sound.currentTime = 0;
                isAudioUnlocked = true;
    ...
    <script>
        window.addEventListener("DOMContentLoaded", function() {
            ...
            // Unlock audio on first user click or keydown
            ...
            // Optional: button for UX
            const enableBtn = document.getElementById("enableSoundBtn");
            //enableBtn.style.display = "inline-block"; // make visible if you want
            enableBtn.addEventListener("click", function() {
                unlockAudio();
                //alert("Notification sound enabled ✅");
                //enableBtn.style.display = "none"; // hide button after clicked
            });

            // Echo listener for gold-orders
            // if (window.Echo) {
            //     window.Echo.channel('gold-orders')
            //         .listen('.new-order', (e) => {
            ...
            //                 sound.play().catch(err => console.log("Play blocked ❌", err));
            //             } else {
            //                 console.log("Audio still locked ❌ (user needs to click first)");
            //             }
            //             const userName = e.order.user ? e.order.user.name : 'Unknown User';
            ...
            //                 icon: 'success',
            //                 title: 'New Order Received',
            //                 text: `User ${userName} submitted ${e.order.amount}`,
            //                 timer: 10000,
            //                 showConfirmButton: false
            //             });
            //         });
            ...
            //             icon: 'success',
            //             title: 'New Order Received',
            //             text: `User ${userName} submitted ${formatNumber(e.order.amount)}`,
            //             timer: 10000,
            //             showConfirmButton: false
            //         });
            //     });
            //     channel.listen('.close-order', (e) => {
            ...
            //             position: 'top-end',
            //             icon: 'success',
            //             title: 'Close Order Received',
            //             text: `User ${userName} Closed PL= ${formatNumber(e.order.pl)}`,
            //             timer: 10000,
            //             showConfirmButton: false
            //         });
            ...
            if (window.Echo) {
                ...
                channel.listen('.new-order', (e) => {
                    ...
                    const userName = e.order.user ? e.order.user.name : 'Unknown User';
                    // 🎯 condition
                    const bgColor = e.order.qty < 0 ? '#ff4d4f' : '#1677ff'; // red : blue
                    const title = e.order.qty < 0 ? 'SELL' : 'BUY'; // red : blue
                    ...
                    Swal.fire({
                        ...
                        title: title,
                       html: `
                            <span style="font-size:22px; font-weight:bold;">${userName}</span>
                            <br>
                            <span style="font-size:18px; font-weight:bold;">
                                ${e.order.qty} ${e.order.currency.sk} x ${e.order.rate} =
                            </span>

                            <span style="font-size:18px; font-weight:bold;">
                                ${formatNumber(e.order.amount)} USD
                            </span>
                        `,
                        timer: 10000,
                        showConfirmButton: false,
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            'exchange_rates'); // here i create a channel or initialize the existing channel
        ...
        function getuseraccount_master(dontshowthaiaccount, refreshmycash, userid) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    op: op,
                    userid: userid,
                    enddingbalance: dontshowthaiaccount
                },
                success: function(data) {
                    console.log(data)
                    if ($.isEmptyObject(data.error)) {
