...
@section('content')
   ...
   <div class="row">
        <table id="myTable" class="table table-bordered tbl_customer kh16" style="table-layout:fixed;">
            ...
            <tbody id="tbl_customer">
                @foreach ($customers as $key => $c)
                    ...
                    <tr>
                        ...
                        <td style="width:120px;">
                            {{ $c->thai_list }} <br> <span>List:{{ $c->showinlist??0 }}</span> <span>Gold:{{ $c->is_gold_list??0 }}</span> <span>ATC:{{ $c->auto_close_list??0 }}</span>
                        </td>
                        {{-- <td style="width:120px;">
         ...
         $(document).on('click','.tbl_partner_account td',function(e){
          ...
          $(document).on('click','.btn_edit_account',function(e){
            ...
            $(document).on('click','.btn_edit',function(e){
                ...
                if(atc==1){
                    $('#auto_closelist').attr('checked',true);
                }else{
                    $('#auto_closelist').attr('checked',false);
                }
            })
