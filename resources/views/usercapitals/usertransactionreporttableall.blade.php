                                    ...
                                    <tbody>
                                      @foreach (App\UserCapital::showlink_ids($ut->link_id,$ut->tablename) as $item)
                                        <tr>
                                            ...
                                            <td style="padding:5px;">
                                              @if(str_contains($item->action,'d'))
                                                <a href="#" class="btn btn-danger btn-sm btndelusercapital" data-id="{{ $item->id }}" data-refnumber="{{ $item->ref_number }}" data-status="{{ $item->status }}" data-location_id="{{ $item->location_id }}"><i class="fa fa-trash"></i></a>
                                                {{-- @if (Auth::user()->role->name<>'Admin')
                                                  @if (App\User::checkpermission(Auth::id(),'1.3.1'))
