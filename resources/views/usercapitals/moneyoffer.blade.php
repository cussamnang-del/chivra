...
@section('css')
    <style type="text/css">
        ...
        #selcur1 + .select2 .select2-selection__rendered {font-family: 'Noto Sans Khmer', sans-serif;font-size:22px;font-weight:bold;}
            /* Each result */
        #select2-selcur1-results {font-family: 'Noto Sans Khmer', sans-serif;font-size:16px;}
        /* Center text vertically in Select2 single select */
        #selcur1 + .select2 .select2-selection--single {
            height: 47px !important;
        }

        #selcur1 + .select2 .select2-selection__rendered {
            line-height: 47px !important;   /* must match height */
            padding-left: 8px;              /* optional */
            padding-right: 30px;            /* keep space for arrow */
        }

        /* Center the arrow too */
        #selcur1 + .select2 .select2-selection__arrow {
            height: 47px !important;
        }

        #selcurkatkong + .select2 .select2-selection__rendered {font-family: 'Noto Sans Khmer', sans-serif;font-size:22px;font-weight:bold;}
            /* Each result */
        #select2-selcurkatkong-results {font-family: 'Noto Sans Khmer', sans-serif;font-size:16px;}

        /* Center text vertically in Select2 single select */
        #selcurkatkong + .select2 .select2-selection--single {
            height: 47px !important;
        }

        #selcurkatkong + .select2 .select2-selection__rendered {
            line-height: 47px !important;   /* must match height */
            padding-left: 8px;              /* optional */
            padding-right: 30px;            /* keep space for arrow */
        }
        ...
        /* Center the arrow too */
        #selcurkatkong + .select2 .select2-selection__arrow {
            height: 47px !important;
        }
    ...
    <script type="text/javascript">
       ...
       $(document).ready(function () {
            ...
            $('#trandate,#showdate,#offerdate').datetimepicker({
             ...
             $("#selcur1").select2({dropdownParent: $("#offerformmodal")});
            $("#selcurkatkong").select2({dropdownParent: $("#offerformmodal")});
            checkright();
        ...
        function runproductrate() {
            ...
            $(document).on('click','#btnoffer',function(e){
                e.preventDefault();
                silentCurrencyChange=true;
                $('#offerformmodal').modal('show');
                $('#frmoffer').trigger('reset');
                $('#selcur1').trigger('change');
                $('#selcurkatkong').trigger('change');
            ...
            $(document).on('keyup','#amount1,#amount2',function(e){
                console.log(this.id);
                if (e.keyCode === 13) {
                ...
                if(isNumber(C)==false){
                   const targetSelect = this.id === 'amount1'
                        ? '#selcur1'
                        : '#selcurkatkong';
                    getcurrencybykeylocalstorage(C, targetSelect);
                }
            })
            function getcurrencybykey(key,el)
            {
            ...
            let silentCurrencyChange = false;
