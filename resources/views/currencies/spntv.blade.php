...
<body>
    ...
    <div id="rate_display_table" class="rotate-wrapper">
            ...
            <tbody>
                @foreach ($cur1->whereIn('shortcut', ['KHR', 'THB', 'THB-KHR']) as $c1)
                    <tr>
                        ...
                        @if ($c1->ispandp == 1)
                            <td class="three-d-text number" style="text-align:left;padding-left:5px;">
                                {{ number_format($c1->ratebuy, $c1->decpoint, '.', '') }}</td>
                            <td class="three-d-text number" style="text-align:right;">
                                {{ number_format($c1->ratesale, $c1->decpoint, '.', '') }}</td>
                        @else
                            <td class="three-d-text number" style="text-align:left;padding-left:5px;">
                                {{ number_format($c1->ratesale, $c1->decpoint, '.', '') }}</td>
                            <td class="three-d-text number" style="text-align:right;">
...
<script>
    ...
    function getratetv() {
        var url = "{{ route('currency.getrate_tv') }}";

        $.get(url, {}, function(data) {
            //console.log(data)
            $('#rate_display_table').empty().html(data);
