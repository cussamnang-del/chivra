        ...
        <tbody id="currencylist">
            @foreach ($currencies as $key => $c)
                <tr>
                    ...
                    <td class="kh16">{{ phpformatnumber($c->buy) }}
                        <br>កក់ទិញលក់
                    </td>
                    <td class="kh16">{{ phpformatnumber($c->sale) }}
                        <br>{{ phpformatnumber($c->buysale_deposit??0) }}
                    </td>
                    ...
                    <td class="td_userconnect" style="@if($c->allow_user==1) color:blue; @else color:red; @endif">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#invdate,#invdate1').datetimepicker({
            ...
            var cleave_buysale_deposit = new Cleave('#buysale_deposit', {
                numeral: true,
                numeralDecimalScale: 6,
                numeralThousandsGroupStyle: 'thousand'
            });
            var cleave_buy = new Cleave('#buy', {
                numeral: true,
                numeralDecimalScale: 6,
                numeralThousandsGroupStyle: 'thousand'
            });
            var cleave_sale = new Cleave('#sale', {
                numeral: true,
                numeralDecimalScale: 6,
                numeralThousandsGroupStyle: 'thousand'
            });
            ...
            var cleave_p_rate = new Cleave('#p_rate', {
                numeral: true,
                numeralDecimalScale: 6,
                numeralThousandsGroupStyle: 'thousand'
            });
        ...
        $(document).on('click','.btnedit',function(e){
                    ...
                    $.get("{{ route('currencyedit') }}",{id:id},function(data){
                        ...
                        $('#buysale_deposit').val(formatNumber(data['currency']['buysale_deposit']??0,data['currency']['decpoint']));
                        if(data['currency']['lmr']=='m'){
                         ...
                         if(data['currency']['allow_user']=='1'){
                            const alowuser = document.getElementById('ck_allow_user');
                            alowuser.checked = true;
                        }else if(data['currency']['allow_user']=='0'){
                            const notallowuser = document.getElementById('ck_notallow_user');
                            notallowuser.checked = true;
                        }
                        ...
                        if(data['currency']['dostock']==1){
                            $('#dostock').prop('checked',true);
                        }else{
                            $('#dostock').prop('checked',false);
                        }
