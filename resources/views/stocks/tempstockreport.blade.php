...
<table class="table table-bordered table-hover tbl_stockreport" style="table-layout:fixed;width:100%;padding:0px;">
  ...
  <tbody id="stockreport">
    ...
    @foreach ($report as $key=>$r)
        ...
        <tr>
            ...
            <td class="kh16-b" style="text-align:center;color:blueviolet;">
                <input type="text" class="form-control kh22-b" name="txtgoldwater[]" value="{{ $r->goldwater }}" style="color:rgb(211, 180, 5);text-align:center;background-color:inherit;border-style:none;" readonly>
            </td>
            <td class="kh22-b" style="text-align:center;">
                <a href="{{ route('stock.viewexchangeprofitdetailbycurrency',['curid'=>$r->currency_id,'tuochek'=>$r->currency->tuochek,'viewdate'=>$r->viewdate,'stockdate'=>$r->stockdate,'todate'=>$r->todate,'userid'=>$r->user_id,'curname'=>$r->currency->curname,'username'=>$r->user->name??'All']) }}" class="kh22-b" style="width:100%;" target="_blank()">{{ $r->currency->curname }} <br> {{  $r->currency->shortcut }}</a>
            </td>
    ...
    <tr style="background-color:aquamarine">
        <td colspan=7 style="text-align:center;font-family:'Noto Sans Khmer', sans-serif;
            font-size:22px;
            font-weight:bold;">សរុបប្រាក់ចំណេញ</td>
        <td  style="text-align:right;font-family:'Noto Sans Khmer', sans-serif;
            font-size:22px;
            font-weight:bold;@if($sumprofit>0) color:blue; @else color:red @endif">
