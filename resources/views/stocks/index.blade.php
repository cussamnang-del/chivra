...
@section('css')
    ...
    .tbl_editstock .clickedrow td input{
        background-color: blue;
        color:white !important;
    }
    /* Full row (header row click) */
    .tbl_editstock .clickedrow td,
    .tbl_editstock .clickedrow td *
    {
        background-color: blue !important;
        color: white !important;
    ...
    /* Single cell (normal row click) */
    .tbl_editstock td.clickedcell {
                ...
                <table class="table table-bordered tbl_stockdate">
                    ...
                    <tbody>
                        @foreach ($stockdates as $stdate)
                            <tr>
                                <td class="kh16-b" style="padding-left:10px;">{{ date('d-m-Y',strtotime($stdate->stockdate)) }}</td>
                                <td class="kh16" style="text-align:center;width:100px;">
                                    <a href="#" class="btnview" data-date="{{ date('d-m-Y',strtotime($stdate->stockdate)) }}">View | </a>
                                    <a href="#" class="btndelete" style="color:red;" data-date="{{ date('d-m-Y',strtotime($stdate->stockdate)) }}">Del</a>
            ...
            <div class="table-responsive">
                <form id="frmstockreport" action="">
                    ...
                    <thead class="kh16" style="text-align:center;">
                            ...
                            <th style="width:100px;">ទឹកមាស</th>
                            <th style="width:200px;">រូបិយប័ណ្ណ</th>
                            <th colspan=2>ស្តុកទំនិញ</th>
...
<script type="text/javascript">
    $('#h1_title').text('ស្តុកលុយ');
       $(document).ready(function () {
        ...
        $(document).on('click','.tbl_editstock td',function(e){
        ...
        function printselectstock(userprint){
          ...
          redirectWindow.location;
                        ...
                        for(let i=0;i<data.length;i++){
                            ...
                            row +=`
                                <tr>
                                    ...
                                    <td style="padding:0px;">
                                        <input type="text" style="border-style:none;width:100%;text-align:center;" name="curid[]" class="form-control kh22" value="${ data[i].id }">
                                    </td>
                                    ...
                                    <td style="padding:0px;">
                                        <input type="text" style="border-style:none;" name="shortcut[]" class="form-control kh22" value="${data[i].curname}">
                                    </td>
                                    <td style="padding:0px;">
                                        <input type="text" style="border-style:none;" name="shortcut[]" class="form-control kh22" value="${data[i].shortcut}">
                                    </td>
                                    <td style="padding:0px;">
                                        <input type="text" style="border-style:none;text-align:right;" name="stock[]" class="form-control tdcanenter stock kh22" value="${formatNumber(data[i].stock)}" autocomplete="off">
                                    </td>
                                    <td style="padding:0px;">
                                        <input type="text" style="border-style:none;text-align:center;" name="rate[]" class="form-control tdcanenter rate kh22" value="${data[i].stock_rate}" autocomplete="off">
                                    </td>
                                    <td style="padding:0px;">
                                        <input type="text" style="border-style:none;text-align:right;" name="amtstock[]" class="form-control tdcanenter amtstock kh22" value="${ formatNumber(data[i].stock_amount)}" autocomplete="off">
                                    </td>
                                    <td style="width:100px;" class="kh22">USD</td>
                                </tr>
                            `;
                        }
                        row += `
                            <tr style="background-color:aqua">
                                <td style="font-size:22px;font-weight:bold;" colspan=6>Total Amount</td>
                                <td id="totalamount" style="text-align:right;font-size:22px;font-weight:bold;">${ formatNumber(total_usd) }</td>
                                <td class="kh22-b">USD</td>
                            </tr>
                        `;
           ...
           $(document).on('click','#btnsavestockedit',function(e){
            ...
            // })
            $(document).on('keyup', '.stock, .rate', function () {
                var row = $(this).closest('tr');
                // Read values inside THIS row only
                var opsign = row.find(".opsign").val();
                var rate = row.find(".rate").val().replace(/,/g, '') || 0;
                var stock = row.find(".stock").val().replace(/,/g, '') || 0;
                ...
                row.find(".amtstock").val(jsformatNumber(amt.toFixed(2)));
                calcTotalAmtStock();
            });
            function calcTotalAmtStock() {
                let total = 0;

                $(".amtstock").each(function () {
                ...
                // UPDATE last-row <td id="totalamount">
                $("#totalamount").text(formatNumber(total.toFixed(2)));
            }
            ...
            // })
            $(document).on('keyup', '.amtstock', function () {
                var row = $(this).closest('tr');
                // Read values from THIS ROW only
                var opsign = row.find('.opsign').val();
                var amtstock = row.find('.amtstock').val().replace(/,/g, '') || 0;
                var stock = row.find('.stock').val().replace(/,/g, '') || 0;

                amtstock = parseFloat(amtstock);
                stock = parseFloat(stock);

                var rate = 0;

                if (stock == 0) {
                    rate = 0; // avoid divide by zero
                } else {
                    if (opsign === '/') {
                        rate = stock / amtstock;
                    } else {
                ...
                row.find('.rate').val(jsformatNumber(rate.toFixed(6)));
                calcTotalAmtStock();
            });
