<script type="text/javascript">
    ...
    function autocomplereceiver() {
        ...
        $("#txtrecphonelist").autocomplete({
            source: sources,
            minLength: 3,
            select: function(event, ui) {
                $("#txtrecphonelist").val(ui.item.value);
                $("#txtrecnamelist").val(ui.item.recname);
                return false;
            }
            //    select : showResult,
            //     focus : showResult,
        ...
        $("#txtrecnamelist").autocomplete({
            source: sources1,
            minLength: 3,
            select: function(event, ui) {
                $("#txtrecnamelist").val(ui.item.value);
                $("#txtrecphonelist").val(ui.item.rectel);
                return false;
            }
    ...
    function autocomplesender() {
        ...
        $("#txtsendphonelist").autocomplete({
            source: sources,
            minLength: 3,
            select: function(event, ui) {
                $("#txtsendphonelist").val(ui.item.value);
                $("#txtsendnamelist").val(ui.item.sendername);
                return false;
            }
            //    select : showResult,
            //     focus : showResult,
        ...
        $("#txtsendnamelist").autocomplete({
            source: sources1,
            minLength: 3,
            select: function(event, ui) {
                $("#txtsendnamelist").val(ui.item.value);
                $("#txtsendphonelist").val(ui.item.sendertel);
                return false;
            }
    ...
    function refreshdisplayrate(isfn, isgold) {
        $('body').addClass("wait");
    ...
    var ably = new Ably.Realtime(
        'DF1ung.N3Jwqw:30ezVuIjqesSJZRbGMoD8NsqtIij6_uqR6soVWetP0Q'); //remember to pass your ably API key
    ...
    channel.subscribe('messageEvent', function(message) { // message this is message from channel
        ...
        if (message.data.customername == domainnameis) {
            refreshdisplayrate(selectisfn, selectisgold);
    ...
    var divheight = windowHeight - 155;
    ...
    function formatOption(option) {
        ...
        var $option = $(
            ...
            '<div class="select2-option-sub" style="font-size:12px;color:red">' + (option.selected ? option.element
                .getAttribute('customertype') : option.element.getAttribute('customertype')) + '</div>' +
            '</div>'
        );
        return $option;
    }
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#btnrefreshrate,#btnrefreshratefn,#btnrefreshrategold', function(e) {
            e.preventDefault();
            var id = $(this).attr('id');
            $('#tblexchangelist').hide();
            $('#tblratedisplay').show();
            if (id == 'btnrefreshrate') {
                selectisfn = 0;
                selectisgold = 0;
            } else if (id == 'btnrefreshratefn') {
                selectisfn = 1;
                selectisgold = 0;
            } else if (id == 'btnrefreshrategold') {
                selectisfn = 0;
                selectisgold = 1;
            }
        ...
        function viewmultiexchangepayment() {
            ...
            `
                myjstable.push({
                    amount: -1 * parseFloat(cashreceive),
                    cur: curbuy
                });
                $('#body_viewmoney').append(row);
            }
        ...
        $(document).on('change', '#ckwater', function(e) {
            ...
            if ($('#selpartner').val() !== '') {
                ...
                fillnextbalance('#balance1', '#balancenext1', $('#selcurlist option:selected').text(),
                    mekun, $('#amtlist').val(), $('#partnerfeelist').val());
            }
        })
        $(document).on('change', '#selcurlist', function(e) {
            ...
            if ($('#selpartner').val() !== '') {
                var mekun = document.querySelector('input[name = optinout]:checked').value;
                if (radv == 'AGENT') {
                    refreshwingratefastnext(totalcash, fillnextbalance, '#balance1', '#balancenext1', $(
                        '#selcurlist option:selected').text(), mekun, $('#amtlist').val(), $(
                        '#partnerfeelist').val());
                } else {
                    fillnextbalance('#balance1', '#balancenext1', $('#selcurlist option:selected')
                        .text(), mekun, $('#amtlist').val(), $('#partnerfeelist').val());
                    totalcash();
                }
            }
        ...
        function refreshwingratefast(callback1, callback2, cid, cur0, elem, elnext, sign0, amt0, fee0,
            fillnextbal) {
            //debugger;
            //try{
        ...
        $(document).on('change', '#selpartner', async function(e) {
            ...
            if (customertype == 'AGENT') {
                ...
                refreshwingratefast(totalcash, getwingbalance, $(this).val(), $(
                        '#selcurlist option:selected').text(), '#balance1', '#balancenext1',
                    mekun, $('#amtlist').val(), $('#partnerfeelist').val(), fillnextbalance);
            } else {
                $('#row_seltranname').css('display', 'none');
                getwingbalance($(this).val(), $('#selcurlist option:selected').text(), '#balance1',
                    '#balancenext1', mekun, $('#amtlist').val(), $('#partnerfeelist').val(),
                    fillnextbalance);
                totalcash();
            }
        ...
        function refill_tbl_in() {
            ...
            $('.action1').each(function(i, e) {
                ...
                $('#txtcur2').val($('#selcurlist option:selected').text().trim());
                var allamt = Math.abs(amt);
                var sumamtlist = sumAmountByCurrency($('#selcurlist option:selected').text().trim());
        ...
        $(document).on('click', '#btnaddtolist', function(e) {
            ...
            var cur1 = $('#selcurlist option:selected').text().trim();
            var curid1 = $('#selcurlist').val();
            var selcurtext = $('#selcurlist option:selected').text().trim();
            var cuschargelist = $('#cuschargelist').val().replace(/,/g, '');
            var cur2 = $('#cuschargelistcur option:selected').text().trim();
            var curid2 = $('#cuschargelistcur').val();
            var feelist = $('#partnerfeelist').val();
            var cur3 = $('#partnerfeelistcur option:selected').text().trim();
            ...
            addrowlist(pid, pname, -1 * parseFloat(transign) * parseFloat(amtlist), cur1, curid1,
                cuschargelist, cur2, curid2, feelist, cur3, curid3, payincash, recnamelist,
                recphonelist, sendnamelist, sendphonelist, transign, useraffect, ex_no, thai_list);
            ...
            if (ex_no != '') {
                if (transign == 1) {
                    if (leftamt <= 0) {
                        updatetbl_out(ex_no, 1);
                    }
                    refill_tbl_out();
                } else {
                    if (leftamt <= 0) {
                        updatetbl_in(ex_no, 1);
                    }
                    refill_tbl_in();
                }
        ...
        function paymentby(s) {
            ...
            if ($('#selpartner').val() !== '') {
                var mekun = document.querySelector('input[name = optinout]:checked').value;
                fillnextbalance('#balance1', '#balancenext1', $('#selcurlist option:selected').text(), mekun, $(
                    '#amtlist').val(), $('#partnerfeelist').val());
            }

        }

        function addrowlist(pid, pname, amt, cur1, idcur1, cuscharge, cur2, idcur2, fee, cur3, idcur3,
            totalcash, recname, rectel, sendname, sendtel, transign, useraffect, ex_no, thai_list) {
            ...
            var row = `<tr class="paybybank">
                      ...
                      <td style="padding:0px;">
                          <input type="text" class="form-control parrent_name kh16-b" style="width:200px;" name="parrent_name[]" value="${pname}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listamt kh16-b" style="text-align:right;width:150px;" name="listamt[]" value="${formatNumber(amt)}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listcur kh16-b" style="width:80px;" name="listcur[]" value="${cur1}" readonly>
                          <input type="hidden" class="form-control tdcanenter listcurid kh12-b" style="width:80px;" name="listcurid[]" value="${idcur1}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listcuscharge kh16-b" style="text-align:right;width:100px;" name="listcuscharge[]" value="${cuscharge}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listcuschargecur kh16-b" style="width:80px;" name="listcuschargecur[]" value="${cur2}" readonly>
                          <input type="hidden" class="form-control tdcanenter listcuschargecurid kh16-b" style="width:80px;" name="listcuschargecurid[]" value="${idcur2}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listfee kh16-b" style="text-align:right;width:100px;" name="listfee[]" value="${fee}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listfeecur kh16-b" style="width:80px;" name="listfeecur[]" value="${cur3}" readonly>
                          <input type="hidden" class="form-control tdcanenter listfeecurid kh16-b" style="width:80px;" name="listfeecurid[]" value="${idcur3}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listtotalcash kh16-b" style="text-align:right;width:150px;" name="listtotalcash[]" value="${totalcash}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listtotalcashcur kh16-b" style="text-align:right;width:80px;" name="listtotalcashcur[]" value="${cur1}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listrectel kh16-b" style="width:200px;" name="listrectel[]" value="${rectel}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listrecname kh16-b" style="width:200px;" name="listrecname[]" value="${recname}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listsendtel kh16-b" style="width:200px;" name="listsendtel[]" value="${sendtel}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter listsendname kh16-b" style="width:200px;" name="listsendname[]" value="${sendname}">
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter transign kh16-b" style="width:100px;" name="transign[]" value="${transign}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter useraffect kh16-b" style="width:200px;" name="useraffect[]" value="${useraffect}" readonly>
                      </td>
                      <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter exno kh16-b" style="width:200px;" name="exno[]" value="${ex_no}" readonly>
                      </td>
                       <td style="padding:0px;">
                          <input type="text" class="form-control tdcanenter thai_list kh16-b" style="width:200px;" name="thai_list[]" value="${thai_list}" readonly>
                      </td>
        ...
        $('#btnaddlist').click(function(e) {
            ...
            if (elg && elg.offsetParent !== null) {
                ...
                if (validate($('#txtbuy').val(), $('#txtsale').val(), $('#txtrate').val(), lomeang) ==
                    1) {
                    alert('invalid exchange')
                    return;
                }
        ...
        function prints(mapid, reprint, isgold_deposit, status = 1) {
            ...
            var redirectWindow = window.open('{{ url('/') }}' + '/exchange/prints?mapid=' + mapid +
                '&reprint=' + reprint + '&isgold_deposit=' + isgold_deposit + '&status=' + status, '_blank');
        ...
        function prints_wait(mapid, reprint, isgold_deposit, status) {
            ...
            return new Promise((resolve) => {
                ...
                table_in.querySelectorAll('tbody tr').forEach((row) => {
                    ...
                    const receive = row.querySelector('input.exmulti_receive_amt')?.value
                        .trim();
                    const returnAmt = row.querySelector('input.exmulti_return_amt')?.value
                        .trim();
                ...
                table_out.querySelectorAll('tbody tr').forEach((row) => {
                    const amt2 = row.querySelector('td.amt2')?.textContent
                        .trim(); // hidden amount
                    const cur2 = row.querySelector('td.cur2')?.textContent
                        .trim(); // hidden currency
                    if (cashreturn == '') {
        ...
        $(document).on('click',
            '#btnsavelist,#btnsavelistprint,#btnsavepartnerlist2,#btnsavepartnerlistprint2,#btnsavedeposit2,#btnsavedepositprint2,#btnprint_test2',
            function(e) {
                e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        success: async function(data) {
                            ...
                            if ($.isEmptyObject(data.error)) {
                                ...
                                if (idclick == 'btnsavelistprint' || idclick ==
                                    'btnsavepartnerlistprint2' || idclick ==
                                    'btnsavedepositprint2' || idclick == 'btnprint_test2') {
        ...
        $(document).on('change', '#type_deposit', function(e) {
            if ($(this).val() == 1) {
                $('#txtdeposit').css('color', 'blue');

            } else {
                $('#txtdeposit').css('color', 'red');
            }
        })
        ...
        $(document).on('click',
            '#btnsave,#btnsaveprint,#btnsavepartnerlist,#btnsavepartnerlistprint,#btnsavedeposit,#btnsavedepositprint,#btnprint_test',
            function(e) {
                e.preventDefault();
                var sp = document.querySelector("#lblsale");
                var lomeang = sp.options[sp.selectedIndex].getAttribute('lomeang');
                ...
                try {
                    ...
                    if ($('#txtcashreceive').val() != '' && $('#txtcashreceive').val() != '0') {
                        cashreceive = $('#txtcashreceive').val() + ' ' + $('#lblcashin option:selected')
                            .text();
                    ...
                    var time = ("0" + today.getHours()).slice(-2) + ":" + ("0" + today.getMinutes()).slice(-
                        2) + ":" + ("0" + today.getSeconds()).slice(-2);
                    var dd = $('#invdate').val();
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if ($.isEmptyObject(data.error)) {
                                if (receipt2 == 1) {
                                    if (btnid == 'btnsaveprint' || btnid ==
                                        'btnsavepartnerlistprint' || btnid ==
                                        'btnsavedepositprint' || btnid == 'btnprint_test') {
        ...
        function refreshexchangelist() {
            ...
            var d1 = $('#invdate').val();
            var d2 = $('#invdate').val();
            var url = "{{ route('getexchangegoldlist') }}";
    ...
    function getcurrencybykey(key, el) {
        ...
        $.get(url, {
            ...
            if (data['c'] != null) {
                ...
                $(el).attr('title', data['c']['id'] + ';' + parseFloat(data['c']['ratebuy']) + ';' + parseFloat(
                        data['c']['ratesale']) + ';' + data['c']['optsign'] + ';' + data['c']['ismain'] +
                    ';' + data['c']['isfn'] + ';' + data['c']['shortcut']);
                getrate();
            }
        })
    }
    ...
    function getcurrencybykeylocalstorage(key, el, imgel) {
        ...
        currencylist.forEach(function(c) {
            ...
            if (c.skey == key) {
                //$(el).val(c.shortcut);
                $(el).val(c.id);
                $(el).attr('title', c.id + ';' + parseFloat(c.ratebuy) + ';' + parseFloat(c.ratesale) + ';' + c
                    .optsign + ';' + c.ismain + ';' + c.isfn + ';' + c.shortcut + ';' + c.isgold + ';' + c
                    .tuochek);
    ...
    function getcurrencybyidlocalstorage(id, el, imgel) {
        ...
        currencylist.forEach(function(c) {
            ...
            if (c.id == id) {
                ...
                $(el).val(c.id);
                $(el).attr('title', c.id + ';' + parseFloat(c.ratebuy) + ';' + parseFloat(c.ratesale) + ';' + c
                    .optsign + ';' + c.ismain + ';' + c.isfn + ';' + c.shortcut);
                $(imgel).attr('src', '{{ config('helper.asset_path') }}/myimages/' + c.imgpath);
                $(imgel).attr('title', c.khname);
                fillcurcur();
                getrate();
    ...
    function docashin() {
        ...
        if ($('#txtsign').val() == '+') {
            ...
            if (isNaN(cashin)) {
                cashreturn = $('#txtsale').val() + ' ' + $('#lblsale option:selected').text().trim();
            } else {
                cashreturn = formatNumber(parseFloat(cashin - luy)) + ' ' + $('#lblbuy option:selected').text().trim();
                if (cashin - luy == 0) {
                    cashreturn = $('#txtsale').val() + ' ' + $('#lblsale option:selected').text().trim();
                } else {
                    cashreturn += "\n" + $('#txtsale').val() + ' ' + $('#lblsale option:selected').text().trim();
                }
            }
            ...
            if (isNaN(cashin)) {
                cashreturn = $('#txtbuy').val() + ' ' + $('#lblbuy option:selected').text().trim();
            } else {
                cashreturn = formatNumber(parseFloat(cashin - luy)) + ' ' + $('#lblsale option:selected').text().trim();
                if (cashin - luy == 0) {
                    cashreturn = $('#txtbuy').val() + ' ' + $('#lblbuy option:selected').text().trim();
                } else {
                    cashreturn += "\n" + $('#txtbuy').val() + ' ' + $('#lblbuy option:selected').text().trim();
                }
            }
            $('#txtcashreturn').val(cashreturn);
    ...
    $("#tableSearch").on("keyup", function() {
        let rawValue = $(this).val().trim().toUpperCase();
        ...
        // 🔹 Detect range: 100-200 or 100.5-2000
        let rangeMatch = value.match(/^(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)$/);
        if (rangeMatch) {
        ...
        $("#tblexchangelist tbody tr").each(function() {
            ...
            $row.find("td").each(function(index) {
                ...
                // 🚫 Skip column 6 (index 5)
                if (index === 5) return;

                let cellText = $(this).text().trim();

                // 🔹 RANGE SEARCH (ABSOLUTE VALUE)
                if (isRange) {
                    let numberMatch = cellText.replace(/,/g, '')
                        .match(/-?\d+(\.\d+)?/);
                    ...
                    if (absNum >= min && absNum <= max) {
                        found = true;
                        return false; // break td loop
                    }
                }
                // 🔹 NORMAL TEXT SEARCH
                else {
                    let searchText = cellText.replace(/,/g, "").toUpperCase();
            ...
            // 🔹 Always hide collapse row
            $row.next(".collapse").removeClass("show");
        });
    });
</script>
