    ...
    <div id="invoice-pos">
        ...
        <div class="row">
            <div class="table-responsive">
                <table class="" style="width:100%;overflow:hidden">
                    <tr>
                        <td class="kh22-b" style="width:100%;text-align:center;padding:0px;text-decoration:underline;">បង្កាន់ដៃបើកវេរ</td>
                    </tr>
                </table>
            </div>
        </div>
                  ...
                  <table id="" class="table tblcashdraw" style="">
                      <tbody>
                          <tr>
                            ...
                            <td class="kh14-b" style="width:75%;text-align:right;">{{ $transfer->frompartner->name }}</td>
                          </tr>
                          <tr>
                              <td class="kh12-b" style="width:25%">ចំនួនទឹកប្រាក់</td>
                              <td class="kh14-b" style="width:75%;text-align:right;">{{ phpformatnumber($transfer->amount) . ' ' . $transfer->currency->shortcut }}</td>
                          </tr>
                          <tr>
                              <td class="kh12-b" style="width:25%">កាត់សេវ៉ា</td>
                              <td class="kh14-b" style="width:75%;text-align:right;">{{ phpformatnumber($transfer->customer_charge) . ' ' . $transfer->cuschargecur->shortcut }}</td>
                          </tr>
                          <tr>
                              ...
                              <td class="kh14-b" style="width:75%;text-align:right;">
                                  @if($transfer->currency_id==$transfer->cuscharge_currency_id)
                                      {{ phpformatnumber($transfer->amount-$transfer->customer_charge). ' ' . $transfer->currency->shortcut }}
                                  @else
                                      {{ phpformatnumber($transfer->amount) . ' ' . $transfer->currency->shortcut }}
                                  @endif
                              </td>
                          </tr>
                          <tr>
                              <td class="kh12-b" style="width:25%">អ្នកទទួល</td>
                              <td class="kh14-b" style="width:75%;text-align:right;">{{ $transfer->receive_tel }} @if($transfer->receive_name) <br> {{ $transfer->receive_name }} @endif</td>
                          </tr>
                      </tbody>
                  </table>
                </div>
                ...
                @if($exchanges->count()>0)
                    @php
                        ...
                        if($transfer->currency_id==$transfer->cuscharge_currency_id){
                          ...
                          if($transfer->cuscharge<>''){
                              $addseva='+ ' .$transfer->cuscharge . ' ' . $transfer->cuschargecur->shortcut;
                            }
                        }
                    ...
                    <table id="tbl_exchange" class="table" style="margin:0px;">
                      ...
                      @foreach ($totalcashdraw_cuscharge as $tc)
                        @if($tc->tcuscharge<>0)
                          <tr>
                            <td style=""></td>
                            <td colspan=2 class="kh12-b" style="text-align:right;">សេវ៉ា</td>
                    ...
                    <table>
                        ...
                        @if($bankpayments->count()>0)
                            @foreach ($bankpayments as $bp)
                                <tr>
                                    <td class="kh12-b" style=""> {{ $bp->partner->name }} @if($bp->cuscharge<>0) +{{phpformatnumber($bp->cuscharge) . $bp->cuschargecur->sk }} @endif</td>
