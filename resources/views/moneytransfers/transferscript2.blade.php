<script type="text/javascript">
    ...
    $('#btnmoveleft').on('click', function (e) {
        ...
        $('#div1').removeClass('col-lg-6').addClass('d-none');
        $('#divdisplay').removeClass('col-lg-6').addClass('col-lg-12');
        $('#btnback').show().text('⇒');
    });
    ...
    $('#btnback').on('click', function () {
        if ($('#div1').hasClass('d-none')) {
            ...
            $('#div1').removeClass('d-none').addClass('col-lg-6');
            $('#divdisplay').removeClass('col-lg-12').addClass('col-lg-6');
    ...
    $(document).ready(function () {
      ...
      $(document).on('change','#amount',function(e){
        ...
        $('#selfee').val($('#s_acc1').val());

      })
        $(document).on('change','#selfee',function(e){
            e.preventDefault();
            var mekun=0;
          var signfee=$(this).val();
          var signtran=$('#s_acc1').val();
          var fee=Math.abs($('#fee').val().replace(/,/g,''));
          if(signtran=='+'){
            if(signfee=='+'){
                mekun=1;
            }else{
          ...
          var xfee=parseFloat(fee)*parseFloat(mekun);
          $('#fee').val(formatNumber(xfee));
            fillbalance();
     ...
     function findRates(agentTypeId, amount, tranNameId, cur) {
      ...
      $(document).on('change','#seluser,#selaccount,#selcursearch',function(e){
        e.preventDefault();
        gettransferlist();
      })
      ...
      function gettransferlist()
      {
        ...
        var selcur=$('#selcursearch').val();
        var selcurtext=$('#selcursearch option:selected').text();
        var url="{{ route('moneytransfer.gettransferlist') }}";
        $.ajax({
                async:true,
                type: 'GET',
                url: url,
                data: {d:d,user_id:user_id,parrent_id:parrent_id,location_id:location_id,selcur:selcur,selcurtext:selcurtext},
