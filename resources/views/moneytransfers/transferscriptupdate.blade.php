<script type="text/javascript">
  ...
  $(document).ready(function () {
      ...
      $(document).on('click','#btnsavetransfer,#btnsavetransferprint',function(e){
        ...
        $(document).on('click','#btnsavetransfer_info,#btnsavetransferprint_info',function(e){
          e.preventDefault();
          $('body').addClass("wait");
          var sp = document.querySelector("#selpartner");
          var customertype=sp.options[sp.selectedIndex].getAttribute('customertype');
          var isprint=0;
          if($(this).attr('id')=='btnsavetransferprint_info'){
              isprint=1;
          }
          var hasexchange=$('#hasexchange').val();
          var hasbankpayment=$('#hasbankpayment').val();
          ...
          var url="{{ route('moneytransfer.justupdateinfo') }}"
          $.ajax({
              async: true,
              type: 'POST',
              contentType: false,
