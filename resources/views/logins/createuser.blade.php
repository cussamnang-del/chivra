...
@section('css')
    ...
    #sellistwith + .select2 .select2-selection__rendered {font-family: 'Noto Sans Khmer', sans-serif;font-size:16px;}
		/* Each result */
	#select2-sellistwith-results {font-family: 'Noto Sans Khmer', sans-serif;font-size:16px;background-color:aquamarine;}
    /* Search field */
        ...
        <tr>
            ...
            <td>
                 <div class="form-check">
                    ...
                    <label class="form-check-label kh22" for="rad_active">Web Users</label>
                </div>
            </td>
            <td>
                 <div class="form-check">
                    <input class="form-check-input kh22" type="radio" name="radact" id="rad_active_mobile" value="2">
                    <label class="form-check-label kh22" for="rad_active_mobile">Mobile Users</label>
                </div>
            </td>
            <td>
                <div class="form-check">
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(document).on('change','.s_active',function(e){
                ...
                $.get(url,{id:id,active:active,col:'active'},function(data){
                    refresh();
                })
            })
             $(document).on('change','.s_activate',function(e){
                var active=$(this).val();
                var id=$(this).data('id');

                var url="{{ route('switchstatus') }}";
                $.get(url,{id:id,active:active,col:'activated'},function(data){
                    refresh();
                })
            })
             $(document).on('change','.s_webcam',function(e){
                var active=$(this).val();
                var id=$(this).data('id');

                var url="{{ route('switchstatus') }}";
                $.get(url,{id:id,active:active,col:'use_camra'},function(data){
                    refresh();
                })
            })
            ...
            $(document).on('click','.user-edit',function(e){
                ...
                $('#selpropertygroup').trigger('change');
                $('#sellistwith').val($(this).data('listwith'));
                $('#sellistwith').trigger('change');
                $('#device_id').val($(this).data('device_id'));
                $('#phone_model').val($(this).data('phone_model'));
                $('#login_device').val($(this).data('login_device'));
                $('#login_phonemodel').val($(this).data('login_phonemodel'));

            })
            $(document).on('change','#rad_active,#rad_disactive,#rad_active_mobile,#selcompany',function(e){
                e.preventDefault();
                refresh();
            })
            function refresh() {
                ...
                $.ajax({
                    ...
                    type: 'GET',
                    url: url,
                    data: {radact:radsel,selcompany:company},
                    success: function (data) {
                        //console.log(data)
                        if($.isEmptyObject(data.error)){
            ...
            $(document).on('click','#btnapplyright',function(e){
                ...
                var uid=$('#us_id').val();
                var company=$('#selcompany').val();
                $.get(url,{uid:uid,company:company},function(data){
