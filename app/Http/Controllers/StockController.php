...
class StockController extends Controller
{
    ...
    public function showstockreport(Request $request){
        ...
        if(isset($request->selgold) && $request->selgold==0){
             $currencies=Currency::where('active',1)->where('ismain',0)->where('ispandp',0)->where('company_id',$selcomid)->where('dostock',1)
             ->where(function($q){
                $q->whereNull('isgold')->orWhere('isgold',0);
             })->orderBy('no')->get();
        }else if(isset($request->selgold) && $request->selgold==1){
             $currencies=Currency::where('active',1)->where('ismain',0)->where('ispandp',0)->where('company_id',$selcomid)->where('isgold',1)->where('dostock',1)->orderBy('no')->get();
        }else{
            $currencies=Currency::where('active',1)->where('ismain',0)->where('ispandp',0)->where('company_id',$selcomid)->where('dostock',1)->orderBy('no')->get();
        }
     ...
     public function doreportgold1(Request $request,$curid,$tuochek,$isexchangecur,$operator,$ratebuy,$fromdate,$viewdate,$todate)
    {
        ...
        if ($stock && $stock->stock !== null) {
            ...
            if ($invbuy->tweight !== null) {
                ...
                if ($buyweight_gold == 0 || $buyamount == 0) {
                    $avgratebuy = 0;  // default value (choose your own)
                } else {
                    if ($operator == '/') {
                        $avgratebuy = $buyweight_gold / $buyamount;
                    } else {
            ...
            if ($invsale->tweight !== null) {
                ...
                // Prevent division by zero
                if ($saleweight_gold == 0 || $saleamount == 0) {
                    $avgratesale = 0; // or null, or "N/A"
                } else {
                    if ($operator == '/') {
                        $avgratesale = $saleweight_gold / $saleamount;
                    } else {
    ...
    public function editstock(Request $request)
    {
        ...
        $currencystock=Currency::where('active',1)->where('ismain',0)->where('ispandp',0)->where('company_id',$selcomid)->orderBy('no')->get();
        // ->where(function($q){
        ...
        {
            $stock = Stock::where('currency_id', $c->id)->latest()->first();
            if($stock){
                $c['stock']=$stock->stock;
