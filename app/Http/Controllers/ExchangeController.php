...
{
    public function refreshdisplayrateinexchangeform(Request $request)
    {
        $selcomid=Session('log_into_company_id');
         $authid=Auth::id();
         if(Auth::user()->role->name<>'Admin'){
              $curleft = Currency::where('active', 1)
                ->where('ispandp', 0)
                ->where('company_id', $selcomid)->where('ismain',0)->where('isfn',$request->isfn)->where('isgold',$request->isgold)
                ->where(function ($query) use ($authid) {

                $query->where(function ($q) use ($authid) {
                    // allow_user = 1 → must be in user_connect
                    $q->where('allow_user', 1)
                    ->whereRaw("FIND_IN_SET(?, user_connect)", [$authid]);
                })
                ->orWhere(function ($q) use ($authid) {
                    // allow_user = 0 → must NOT be in user_connect
                    $q->where('allow_user', 0)
                    ->where(function ($sub) use ($authid) {
                        $sub->whereNull('user_connect')
                            ->orWhereRaw("NOT FIND_IN_SET(?, user_connect)", [$authid]);
                    });
                })
                ->orWhere(function ($q) use ($authid) {
                    // allow_user = 0 → must NOT be in user_connect
                    $q->where('allow_user', 1)->where('isfn',0);
                });
            })
            ->orderBy('no')->get();
         }else{
             $curleft=Currency::where('active',1)->where('ismain',0)->where('isfn',$request->isfn)->where('isgold',$request->isgold)->where('company_id',$selcomid)->orderBy('no')->get();
         }
   ...
   public function index()
   {
    ...
    $totalsale=DB::table('exchange_multis')->select(DB::raw('sum(sale) as tsale,cursale'))
      ...
      if(Auth::user()->role->name<>'Admin'){
        ...
        //     })
        //     ->get();
        $allcurrencies = Currency::where('active', 1)
        ->where('ispandp', 0)
   ...
   public function savedepositgold(Request $request)
   {
    ...
    public function savedepositgold2(Request $request)
   {
        //return $request->all();
        $validator = Validator::make($request->all(), [
          'txtdeposit2' => 'required', //input array validate
          'txtdeposit3' => 'required', //input array validate
        ...
        $current->timezone('Asia/Phnom_Penh');
        $invtime = date("H:i:s",strtotime($current));
        if(isset($request->total_exid)){
            $groupid='exchange-'.$request->total_exid;
        }else{
            $groupid='';
        }
        $date = str_replace('/', '-', $request->deposit_date2);
        $dd= date('Y-m-d', strtotime($date));
        $this->savecustomergoldlist2($request,$dd,$invtime,$groupid,$current);

        if(isset($request->total_exid)){
            $groupid=explode('-',$request->total_exid);
            DB::table('exchanges')->whereIn('id',$groupid)->update(['gold_payment_group'=>'exchange-'.$request->total_exid]);
        }
   }
   public function saveexchange_old(Request $request)
   {
        $selcomid=Session('log_into_company_id');
      //return $request->all();
    ...
    try{
        ...
        if($e->save()){
            ...
            if($request->status==1){
                ...
                return response()->json(['success'=>'save completed','id'=>$refgroupid]);
            }
    ...
    public function saveexchange(Request $request)
    {
        ...
        // ================= TRANSACTION START =================
        // Everything from here to commit will be "undone" if an error happens
        DB::beginTransaction();

        try {
            $current = Carbon::now('Asia/Phnom_Penh');
            $invtime = $current->format('H:i:s');
            $invtime1 = Carbon::parse($invtime)->addSecond()->format('H:i:s');
            $date = str_replace('/', '-', $request->dd);
            $exchangedate = date('Y-m-d', strtotime($date));
            ...
            if ($request->foundpartnerlist == '1') {
                foreach ($request->partner_id as $key => $value) {
                    $amt = ($request->listamt[$key] ?? 0) . ' ' . ($request->listcur[$key] ?? '');
                    $pname = $request->parrent_name[$key] ?? '';
                    $exnote .= ($exnote == '' ? '' : ' ') . $pname . '(' . $amt . ')';
                }
            }
            ...
            // ================= COMMIT =================
            // Only now does the data actually stay in the database
            DB::commit();

            return response()->json([
                'success' => true,
                'id' => $request->status == 1 ? $e->id : $refgroupid
            ]);

        } catch (\Throwable $th) {
            // ================= ROLLBACK =================
            // If ANY error happened above, this deletes everything from this request
            DB::rollBack();
   ...
   public function saveexchangeproduct_old(Request $r)
   {
      //return $r->all();
    ...
    public function saveexchangeproduct(Request $r)
   {
      //return $r->all();
        ...
        try{
            ...
            if ($r->foundpartnerlist == '1') {
                foreach ($r->partner_id as $key => $value) {
                    $amt = ($r->listamt[$key] ?? 0) . ' ' . ($r->listcur[$key] ?? '');
                    $pname = $r->parrent_name[$key] ?? '';
                    $exnote .= ($exnote == '' ? '' : ' ') . $pname . '(' . $amt . ')';
                }
            }
   ...
   public function getuseraffectbank_old($cid){
   ...
   public function customerpayexchangebypartner(Request $request,$trdate,$trtime,$ref_number,$current)
   {
       ...
       foreach ($request->partner_id as $key => $value) {
           ...
           if($ptf->save()){
               ...
               //save to buysalegold
               if(config('helper.hasgoldapp') == '1' && $customer->customertype=='CUSTOMER'){
                    $amt_bs=floatval($request->transign[$key]) * floatval($amt);
                    $buysale=new BuySaleGold();
                    $buysale->customer_id=$value;
                    $buysale->user_id=Auth::id();
                    $buysale->dd=$trdate;
                    $buysale->tt=$trtime;
                    $buysale->currency_id=$request->listcurid[$key];
                    $buysale->qty=0;
                    $buysale->rate=0;
                    $buysale->amount= $amt_bs;
                    $buysale->price_last=0;
                    $buysale->is_close=1;
                    $buysale->gold_req=0;
                    $buysale->deposit=0;
                    $buysale->pl=0;
                    $buysale->is_ptr=1;
                    $buysale->tran_group=$ref_number;
                    $buysale->tran_name= $amt_bs >=0 ? 'ដាក់ប្រាក់' : 'ដកប្រាក់';
                    $buysale->save();
               }
           }
       }
    ...
    public function savecustomergoldlist(Request $request,$trdate,$trtime,$ref_number,$current,$amount,$luy_id,$exsign)
   {
        ...
        if($ptf->save()){
            ...
            if (!empty($request->selbankdeposit) && $request->selbankdeposit>0) {
                ...
                // }
                if($request->type_deposit==1){ //យើងកក់គេ
                    $ptf2->tranname='បាញ់ចេញ';
                    $ptf2->trancode=4;
                    $ptf2->mekun=1;
                    $ptf2->amount=str_replace(',','',$request->txtdeposit1);
   ...
   public function savecustomergoldlist1(Request $request,$trdate,$trtime,$ref_number,$current)
   {
    ...
    public function savecustomergoldlist2(Request $request,$trdate,$trtime,$ref_number,$current)
   {
        $selcomid=Session('log_into_company_id');
        $wepaid=0;
        if($request->paidform=='goldlist'){
            if(floatval(str_replace(',','',$request->txtdebt2))<0){
                $wepaid=0;
            }else{
                $wepaid=1;
            }
        ...
        if (!empty($request->selbankdeposit2)) {
                //លុយកក់របស់អតិថិជន
                    $ptf = new PartnerTransfer();
                    $ptf->company_id=$selcomid;
                    if($wepaid==1){
                        ...
                        $ptf->trancode=-4;
                        $ptf->mekun=-1;
                        $ptf->amount=-1 * floatval(str_replace(',','',$request->txtdeposit3));
                    }else{
                        $ptf->tranname='ផ្ញើ';
                        $ptf->trancode=4;
                        $ptf->mekun=1;
                        $ptf->amount=str_replace(',','',$request->txtdeposit3);
                    }
                    $ptf->dd=$trdate;
                    $ptf->tt=$trtime;
                    $ptf->user_id=Auth::id();
                    $ptf->parrent_id=$request->selcustomergold2;
                    $ptf->currency_id=$request->selcur2;
                    $ptf->iscutwater=0;
                    $ptf->cuscharge=0;
                    $ptf->cuscharge_currency_id=$request->selcur2;
                    $ptf->cuscharge_ex=0;
                    $ptf->fee=0;
                    $ptf->interest=0;
                    $ptf->fee_currency_id=$request->selcur2;
                    $ptf->fee_ex=0;
                    $ptf->bonus=0;
                    $ptf->sendername='';
                    $ptf->sendertel='';
                    $ptf->recname=$request->client_name2;
                    $ptf->rectel=str_replace(' ','',$request->client_tel2);
                    $ptf->ref_group_id=$ref_number;
                    $ptf->ref_number=$ref_number;
                    $ptf->note='កក់មាស';
                    $ptf->deposit=str_replace(',','',$request->txtdeposit2);
                    $ptf->deposit_via=$request->deposit_via;
                    $ptf->created_at=$current;
                    $ptf->updated_at=$current;
                    $ptf->action='d';
                    ...
                    if($ptf->save()){
                         $id=$ptf->id;
                        if($ref_number==''){
                            $ref_number='goldlist-' . $id;
                            DB::table('partner_transfers')->where('id',$id)->update(['ref_group_id'=>$ref_number]);
                        }
                        // user selected bank
                        $ptf2 = new PartnerTransfer();
                        $ptf2->company_id=$selcomid;
                        if($wepaid==1){
                            ...
                            $ptf2->trancode=4;
                            $ptf2->mekun=1;
                            $ptf2->amount=str_replace(',','',$request->txtdeposit3);
                        }else{
                            $ptf2->tranname='បាញ់ចូល';
                            $ptf2->trancode=-4;
                            $ptf2->mekun=-1;
                            $ptf2->amount=-1 * floatval(str_replace(',','',$request->txtdeposit3));
                        }
                        $ptf2->dd=$trdate;
                        $ptf2->tt=$trtime;
                        $ptf2->user_id=Auth::id();
                        $ptf2->parrent_id=$request->selbankdeposit2;

                        $ptf2->currency_id=$request->selcur2;
                        $ptf2->iscutwater=0;
                        $ptf2->cuscharge=0;
                        $ptf2->cuscharge_currency_id=$request->selcur2;
                        $ptf2->cuscharge_ex=0;
                        $ptf2->fee=0;
                        $ptf2->interest=0;
                        $ptf2->fee_currency_id=$request->selcur2;
                        $ptf2->fee_ex=0;
                        $ptf2->bonus=0;
                        $ptf2->sendername='';
                        $ptf2->sendertel='';
                        ...
                        $ptf2->ref_group_id=$ref_number;
                        $ptf2->ref_number=$ref_number;
                        $ptf2->note='ទូទាត់បញ្ជីមាស';
                        $ptf2->created_at=$current;
                        $ptf2->updated_at=$current;
                        $ptf2->location_id=1;
                        if($request->customertype_deposit=='AGENT' || $request->customertype_deposit=='BANK'){
                            $ptf2->user_affect=$this->getuseraffectbank($request->selbankdeposit2);
                        }
                        $ptf2->save();

                        if($request->cash_amount>0){
                           $this->savecustomergoldpaycash2($request,$trdate,$trtime,$ref_number,$current,$wepaid);
                        }
                    }
            } else {
                // user selected cash
   ...
   public function savecustomergoldpaycash1(Request $request,$trdate,$trtime,$ref_number,$current,)
   {
    ...
    public function savecustomergoldpaycash2(Request $request,$trdate,$trtime,$ref_number,$current,$wepaid)
   {
        $selcomid=Session('log_into_company_id');
        $ptf3 = new PartnerTransfer();
        $ptf3->company_id=$selcomid;
        if($wepaid==1){
            ...
            $ptf3->tt=$trtime;
            $ptf3->user_id=Auth::id();
            $ptf3->parrent_id=$request->selcustomergold2;

            $ptf3->currency_id=$request->selcur2;
            $ptf3->iscutwater=0;
            $ptf3->cuscharge=0;
            $ptf3->cuscharge_currency_id=$request->selcur2;
            $ptf3->cuscharge_ex=0;
            $ptf3->fee=0;
            $ptf3->interest=0;
            $ptf3->fee_currency_id=$request->selcur2;
            $ptf3->fee_ex=0;
            $ptf3->bonus=0;
            $ptf3->sendername='';
            $ptf3->sendertel='';
            $ptf3->recname=$request->client_name2;
            $ptf3->rectel=str_replace(' ','',$request->client_tel2);
            $ptf3->ref_group_id=$ref_number;
            $ptf3->ref_number=$ref_number;
            $ptf3->note='ទូទាត់មាស';
            $ptf3->deposit=str_replace(',','',$request->cash_amount);
            ...
            if($ptf3->save()){
                $id3=$ptf3->id;
                if($ref_number==''){
                        $ref_number='goldlist-' . $id3;
                        DB::table('partner_transfers')->where('id',$id3)->update(['ref_group_id'=>$ref_number]);
                    }
                if($savecashdraw==1){
                    $cashdraw=new Cashdraw();
                    $cashdraw->transfer_id=$id3;
                    $cashdraw->from_partner_id=$request->selcustomergold2;
                    $cashdraw->opdate=$trdate;
                    $cashdraw->optime=$trtime;
                    $cashdraw->user_id=Auth::id();
                    $cashdraw->amount= $request->cash_amount;
                    $cashdraw->currency_id=$request->selcur2;
                    $cashdraw->customer_charge=0;
                    $cashdraw->paymethod='Cash';
                    $cashdraw->receive_tel=str_replace(' ','',$request->client_tel2);
                    $cashdraw->receive_name=$request->client_name2;
                    $cashdraw->note='ទូទាត់បញ្ជីមាស';
                    $cashdraw->other='';
                    $cashdraw->ref_number=$ref_number;
                    $cashdraw->ref_group_id=$ref_number;
                    $cashdraw->created_at=$current;
  ...
  public function exchangeprints(Request $request){
    //return $request->all();
      $selcomid=Session('log_into_company_id');
        ...
        if($request->isgold_deposit==1){
            ...
            if($request->status==1){
                ...
                if (str_contains($request->mapid, 'exchange-')) {
                    $groupid=$request->mapid;
                } else {
                    if(str_contains($request->mapid,'printtest')){
                        $groupid=$request->mapid;
                    }else{
                        $groupid='exchange-' . $request->mapid;
                    }
                }
                $exchanges=ExchangeMulti::where('ref_group_id',$groupid)->where('company_id',$selcomid)->orderBy('id')->get();
                $totalbuy=DB::table('exchange_multis')->select(DB::raw('sum(buy) as tbuy,curbuy'))->where('ref_group_id',$groupid)->where('company_id',$selcomid)->groupBy('curbuy')->get();
                $totalsale=DB::table('exchange_multis')->select(DB::raw('sum(sale) as tsale,cursale'))->where('ref_group_id',$groupid)->where('company_id',$selcomid)->groupBy('cursale')->get();
                $bankpayments=PartnerTransfer::where('ref_group_id','=',$groupid)->where('company_id',$selcomid)->get();
            }
      ...
      if($request->isgold_deposit==1){
          ...
          if(config('helper.transfer_option') == 'norn'){
            ...
            return view('exchanges.receipt_chivra',compact('exchanges','totalbuy','totalsale','logo','bankpayments','cash','cashreceive_cm','cashreturn_cm','reprint','status'));
    ...
    public function exchangegoldreport(Request $request)
   {
      ...
      $userid=Auth::user()->id;
      $partners=Customer::where('status',1)->where('company_id',$selcomid)->orderBy('no')->get();
      $exchanges=Exchange::where('user_id',$userid)->whereDate('dd',$dd)
      ->where('status',1)->where('isexchangelist',0)->where('company_id',$selcomid)->where('processing',1)->whereNull('gold_payment_group')->orderBy('id')->get();
      ...
      ->where('status',1)->where('isexchangelist',0)->where('company_id',$selcomid)->whereDate('dd',$dd)
      ->where(function($q){
        $q->where('processing',2)->orWhereNotNull('gold_payment_group');
      })->orderBy('id')->get();
    ...
    public function exchangegoldlist(Request $request)
   {
      $selcomid=Session('log_into_company_id');
      $current = Carbon::now();
      $current->timezone('Asia/Phnom_Penh');
      ...
      ->where('status',1)->where('isexchangelist',0)->where('company_id',$selcomid)->where('processing',1)->whereNull('gold_payment_group')->whereDate('dd',$dd)->orderBy('id')->get();
      foreach($exchanges as $e){
        $sumamount=Exchange::where('status',1)->where('ref_group_id',$e->ref_group_id)->sum('amount');
        $balance=PartnerTransfer::where('status',1)->where('ref_group_id',$e->ref_group_id)->where('rectel',$e->phone)->sum('amount');
        if($e->id==$e->multiexchangecode){
      ...
      return view('exchanges.exchangegoldlist',compact('exchanges','exchanges_complete','users','partners','currencies'));
   }
     public function getexchangelistgold(Request $request)
   {
      ...
      if(!empty($request->userid)){
         $exchanges=Exchange::where('user_id',$request->userid)
          ->where('status',1)->where('isexchangelist',0)->where('company_id',$selcomid)->where('processing',1)->whereNull('gold_payment_group')->whereBetween(DB::raw('DATE(updated_at)'), array($d1, $d2));
          $exchanges_complete=Exchange::where('user_id',$request->userid)->whereBetween(DB::raw('DATE(updated_at)'), array($d1, $d2))
          ->where('status',1)->where('isexchangelist',0)->where('company_id',$selcomid)
          ->where(function($q){
            $q ->where('processing',2)->orWhereNotNull('gold_payment_group');
          });
      }else{
          $exchanges=Exchange::where('status',1)->where('isexchangelist',0)->where('company_id',$selcomid)->where('processing',1)->whereNull('gold_payment_group')->whereBetween(DB::raw('DATE(updated_at)'), array($d1, $d2));
   ...
   public function getgoldlist(Request $request){
       ...
       $sumtransfer = PartnerTransfer::where('parrent_id', $request->customer)
        ->where('status', 1)->whereNull('clear_date')
        ->selectRaw('SUM(amount) as total, currency_id, recname, rectel')
        ->groupBy('currency_id', 'recname', 'rectel')
        //->having('total', '<>', 0)
        ->get()
        ->load('currency');

        return response()->json(['sumtransfer'=>$sumtransfer]);

    }
    public function getgoldlistdetail(Request $request){
        //return $request->all();
        $transfers=PartnerTransfer::where('status',1)->where('parrent_id',$request->customer)->where('rectel',$request->rectel)->where('recname',$request->recname)->whereNull('clear_date')->orderBy('id')->get();
        return view('exchanges.getgoldlistdetail',compact('transfers'));
    }
    public function cleargoldlist(Request $request){
        //return $request->all();
        $current = Carbon::now();
        $current->timezone('Asia/Phnom_Penh');
        $dd = date("y-m-d",strtotime($current));
         $sumtransfer = PartnerTransfer::where('parrent_id', $request->customer)->where('rectel',$request->rectel)->where('recname',$request->recname)
        ->where('status', 1)->whereNull('clear_date')->sum('amount');
        if($sumtransfer==0){
            DB::table('partner_transfers')->where('parrent_id', $request->customer)->where('rectel',$request->rectel)->where('recname',$request->recname)
            ->where('status', 1)->whereNull('clear_date')->update(['clear_date'=>$dd,'clear_by'=>Auth::user()->name]);
            return response()->json(['success'=>true]);
        }else{
             return response()->json(['success'=>false]);
        }
    }
      public function deletecleargoldlist(Request $request){

        $update= DB::table('partner_transfers')->where('parrent_id', $request->customer)->where('rectel',$request->rectel)->where('recname',$request->recname)
        ->where('status', 1)->where('clear_date',$request->cleardate)->where('clear_by',$request->clearby)->update(['clear_date'=>null,'clear_by'=>null]);
        if($update){
    ...
    public function getcleardate(Request $request){
        $data = DB::table('partner_transfers')
        ->select(
            'recname',
            'rectel',
            DB::raw('MAX(clear_date) as clear_date'),
            DB::raw('MAX(clear_by) as clear_by')
        )
        ->whereNotNull('clear_date')
        ->whereNotNull('clear_by')
        ->where('status',1)
        ->groupBy('recname', 'rectel')
        ->get();
        ...
        if($es){
            ...
            if($e){
                ...
                if(config('helper.hasgoldapp') == '1'){
                    DB::table('buy_sale_gold')->where('tran_group',$refnumber)->update(['status'=>0,'del_by'=>Auth::id(),'updated_at'=>$current]);
                }
                return response()->json(['success'=>true,'message'=>'this exchange has been removed.']);
            }
        }
   }
   public function deletegoldpayment(Request $request){
        $prefix_group = explode('-', $request->group_id)[0];
        if ($prefix_group === 'exchange') {

            // Cancel all partner transfers under this exchange group
            ...
            // Cancel cash draw related to this exchange group
            DB::table('cashdraws')
                ->where('ref_group_id', $request->group_id)
                ->update([
                    'status' => 0,
                    'delby' => Auth::user()->name
                ]);

            // Remove gold payment group reference from exchanges
            DB::table('exchanges')
                ->where('gold_payment_group', $request->group_id)
                ->update([
                    'gold_payment_group' => null
                ]);

        } else {

            // For gold payment single row
            $groupid = 'goldpayment-' . $request->id;

            DB::table('partner_transfers')
                ->where('id', $request->id)
                ->orWhere('ref_group_id', $groupid)
                ->update([
                    'status' => 0,
                    'user_delete' => Auth::id()
                ]);
     ...
     return response()->json(['success'=>true,'message'=>'this payment has been removed.']);
   }
