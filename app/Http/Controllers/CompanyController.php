...
class CompanyController extends Controller
{
    ...
    if ($validator->fails()) {
        return response()->json(['error'=>$validator->errors()->all()]);
            ...
            $nb->facebook=$request->facebook;
            $nb->telegram=$request->telegram;
            $nb->tel1=$request->tel1;
            $nb->tel2=$request->tel2;
            $nb->tel3=$request->tel3;
            $nb->timework=$request->timework;
            $nb->inv_gold_note=$request->goldnote;
            ...
            if ($request->hasFile('image')) {
                ...
                if ($nb->logo && file_exists(public_path('logo/'.$nb->logo))) {
                    unlink(public_path('logo/'.$nb->logo));
                }

                $image = $request->file('image');

                // Generate clean filename
                $imgname = time().'_'.Str::random(8).'.'.$image->extension();
