...
class CurrencyController extends Controller
{
    ...
    public function setrate_old(Request $request)
    {
        //return($request->all());
        $current = Carbon::now();
        $current->timezone('Asia/Phnom_Penh');
    ...
    public function updateBuySaleGoldPL($cur_id,$buy,$sale,$ratebuy,$ratesale)
    {
        ...
        $warningUserIds = DB::table('buy_sale_gold')
        ...
        $emptyUserIds = DB::table('buy_sale_gold')
                ->whereRaw('(pl + deposit) <= 0')
                ->pluck('user_id')
                ->unique()
                ->toArray();

            if (!empty($warningUserIds)) {
                $tokens = \App\Models\User::whereIn('id', $warningUserIds)
            ...
            if (!empty($emptyUserIds)) {
                $tokens = \App\Models\User::whereIn('id', $emptyUserIds)
                    ->whereNotNull('fcm_token')
                    ->pluck('fcm_token')
                    ->toArray();
     ...
     public function setrate(Request $request)
    {
        ...
        foreach ($request->curid as $key => $value) {
            ...
            $ratebuy   = str_replace(',', '', $request->ratebuy[$key]);
            $ratesale  = str_replace(',', '', $request->ratesale[$key]);
            $exists = ExchangeRate::where('currency_id', $value)
            ...
            if (!$exists) {
                // 1️⃣ Not exists today → save
                $shouldSave = true;
            } else {
                // 2️⃣ Exists → compare values
                if (
                    $exists->buy       != $buy ||
                    $exists->sale      != $sale ||
                    $exists->ratebuy   != $ratebuy ||
                    $exists->ratesale  != $ratesale
                ) {
                    $shouldSave = true;
                }
            }
        ...
        if($count>0){
            ...
            return response()->json(['success'=>'true','message'=>'rate have been set successfully.','type'=>'1']);
        }else{
    ...
    public function setratethai(Request $request)
    {
        ...
        foreach ($request->curname as $key => $value) {
            ...
            $exists = ThaiRateSet::where('curname', $value)
                ->whereDate('dd', $current)
                ->where('company_id', $selcomid)
                ->where('exchangetype',$request->exchangetype)
                ->latest('id')
                ->first();
        ...
        if($count>0){
            return response()->json(['success'=>'true','message'=>'rate have been set.','type'=>'1']);
        }else{
            return response()->json(['success'=>'true','message'=>'no rate thai set.','type'=>'0']);
        }
    ...
    public function ratedisplayforgold_old()
    {
        // $current = Carbon::now();
        // $current->timezone('Asia/Phnom_Penh');
        // $today = date("y-m-d",strtotime($current));
        // $yesterday = $current->subDay()->format('Y-m-d'); // 2026-02-09

        $tz = 'Asia/Phnom_Penh';
        $current = Carbon::now($tz);
        $today = $current->copy();            // Carbon
        $yesterday = $current->copy()->subDay(); // Carbon
        //return $yesterday;
        // 1️⃣ Get all today's gold rates
        $todayRates = ExchangeRate::join('currencies', 'currencies.id', '=', 'exchange_rates.currency_id')
            ->whereDate('exchange_rates.created_at', $today)
            ->where('currencies.isgold', 1)
            ->orderBy('exchange_rates.created_at')
            ->get([
                'exchange_rates.id',
                'exchange_rates.ratebuy',
                'exchange_rates.ratesale',
                'exchange_rates.buy',
                'exchange_rates.sale',
        ...
        //return $todayRates;
        // 2️⃣ Get yesterday LAST rate per currency
        $yesterdayLastRates = ExchangeRate::join('currencies', 'currencies.id', '=', 'exchange_rates.currency_id')
            ->whereDate('exchange_rates.created_at', $yesterday)
            ->where('currencies.isgold', 1)
            ->whereIn(
                'exchange_rates.id',
                ExchangeRate::whereDate('created_at', $yesterday)
                    ->selectRaw('MAX(id)')
                    ->groupBy('currency_id')
            )
            ->get([
        ...
        // ================================
        // 3️⃣ Merge logic PER currency
        // ================================
        $rates = collect();
        $todayGrouped = $todayRates->groupBy('currency_id');

        foreach ($todayGrouped as $currencyId => $currencyRates) {

            // CASE A: only 1 record today → prepend yesterday last
            if ($currencyRates->count() === 1 && isset($yesterdayLastRates[$currencyId])) {
                $rates->push($yesterdayLastRates[$currencyId]);
            }
        ...
        // ================================
        // 4️⃣ No today record → use yesterday
        // ================================
        if ($todayRates->isEmpty()) {
            $rates = $yesterdayLastRates->values();
            $displayDate = $yesterday->toDateString();
        } else {
            $displayDate = $today->toDateString();
        }

        // ================================
        // 5️⃣ NEW: build LAST vs PREVIOUS summary
        // ================================
        $summaryRates = [];

        $rates->groupBy('currency_id')->each(function ($items, $currencyId) use (&$summaryRates) {

            $items = $items->sortBy('created_at')->values();

            $last = $items->last();
            $prev = $items->count() > 1 ? $items[$items->count() - 2] : null;

            $summaryRates[$currencyId] = [
                'currency_id' => $currencyId,
                'curname'     => $last->curname,
                ...
                'buy_trend'  => $prev
                    ? ($last->buy > $prev->buy ? 'up' : ($last->buy < $prev->buy ? 'down' : 'same'))
                    : 'same',

                'sale_trend' => $prev
                    ? ($last->sale > $prev->sale ? 'up' : ($last->sale < $prev->sale ? 'down' : 'same'))
                    : 'same',

                'updated_at' => $last->created_at->format('Y-m-d H:i:s'),
            ];
        });
        $latestUpdatedAt = collect($summaryRates)->max('updated_at');
        ...
        //return $rates;
        return view(
            'currencies.goldratedisplay4',
            compact('rates', 'summaryRates', 'displayDate','latestUpdatedAt','company')
        );
    }
     public function ratedisplayforgold()
    {
        // $current = Carbon::now();
        // $current->timezone('Asia/Phnom_Penh');
        // $today = date("y-m-d",strtotime($current));
        ...
        $today = $current->copy();            // Carbon
        $yesterday = $current->copy()->subDay(); // Carbon
        // Find last available date before today
        $lastAvailableDate = ExchangeRate::join('currencies', 'currencies.id', '=', 'exchange_rates.currency_id')
            ->where('exchange_rates.created_at', '<', $today->startOfDay())
            ->where('currencies.isgold', 1)
            ->max('exchange_rates.created_at');

        $previousDate = $lastAvailableDate
            ? Carbon::parse($lastAvailableDate)->toDateString()
            : null;
        //return $yesterday;
        // 1️⃣ Get all today's gold rates
        $todayRates = ExchangeRate::join('currencies', 'currencies.id', '=', 'exchange_rates.currency_id')
        ...
        $yesterdayLastRates = collect();

        if ($previousDate) {

            $yesterdayLastRates = ExchangeRate::join('currencies', 'currencies.id', '=', 'exchange_rates.currency_id')
                ->whereDate('exchange_rates.created_at', $previousDate)
                ->where('currencies.isgold', 1)
                ->whereIn(
                    'exchange_rates.id',
                    ExchangeRate::whereDate('created_at', $previousDate)
                        ->selectRaw('MAX(id)')
                        ->groupBy('currency_id')
                )
                ->get([
        ...
        //return $rates;
        return view(
            'currencies.goldratedisplay5',
            compact('rates', 'summaryRates', 'displayDate','latestUpdatedAt','company')
        );
    }

    public function goldChartData(Request $request)
    {
        $tz = 'Asia/Phnom_Penh';
        ...
        //     ? Carbon::parse($selectedDate, $tz)
        //     : Carbon::now($tz);

         if ($request->dd) {
            $baseDate = Carbon::createFromFormat('d-m-Y', $request->dd, $tz)
                            ->startOfDay();
        } else {
            $baseDate = Carbon::now($tz)->startOfDay();
        }
        //return $baseDate;
        $query = ExchangeRate::join('currencies', 'currencies.id', '=', 'exchange_rates.currency_id')
            ->where('currencies.isgold', 1)
            ->select(
                'exchange_rates.id',
                'exchange_rates.ratebuy',
        ...
        if ($filter === 'day') {
            ...
            $rates = $query
                ->whereDate('exchange_rates.created_at', $baseDate->toDateString())
                ->orderBy('exchange_rates.created_at')
                ->get()
                ->map(function ($item) use ($tz) {

                    $item->label = Carbon::parse($item->created_at)
                        ->timezone($tz)
                        ->format('H:i');
        ...
        //     $start = $baseDate->copy()->startOfWeek(); // week of selected date
        //     $end   = $baseDate->copy()->endOfWeek();

        //     $rates = $query
        //         ->whereBetween('exchange_rates.created_at', [$start, $end])
        //         ->orderBy('exchange_rates.created_at')
        //         ->get()
        ...
        //             return $dayRates->last(); // closing rate per day
        //         })
        //         ->values()
        //         ->map(function ($item) use ($tz) {
        ...
        // }
        elseif ($filter === 'week') {
            $start = $baseDate->copy()->startOfWeek(); // start of week
            $end   = $baseDate->copy()->endOfWeek();   // end of week
        ...
        // ====================================================
        // 🔵 MONTH VIEW (month of selected date)
        // ====================================================
        // else {
        //     // 🔵 MONTH VIEW (month of selected date)
        ...
        //             return $item->currency_id . '_' . Carbon::parse($item->created_at)
        //                 ->timezone($tz)
        //                 ->format('d');
        //         })
        //         ->map(function ($dayRates) {
        //             return $dayRates->last(); // closing rate per day per currency
        //         })
        //         ->values()
        //         ->map(function ($item) use ($tz) {
        //             $item->label = Carbon::parse($item->created_at)
    ...
    public function ratedisplayforcustomer($isfn,$isgold)
    {
        //return $isfn;
        if (isset($_COOKIE['company_id_cookie'])) {
            $selcomid = $_COOKIE['company_id_cookie'];
        ...
        }else if(config('helper.transfer_option') == 'tangheng'){

             $currencies =Currency::where('company_id',$selcomid)
                ->where('active', 1)->where('isfn',$isfn)
                ->orderBy('no')
                ->get();
            return view('currencies.ratedisplayfortangheng',compact('currencies'));
        }else{
    ...
    public function ratedisplayforcustomer_sendToSocial()
{
    ...
    //$path = storage_path('app/' . config('helper.transfer_option') . '/' . $filename);
    ...
    protected function sendToTelegramGroup($imagePath)
{
    if(config('helper.transfer_option') == 'kuongkea'){
        $token = config('services.telegram.kuongkea_bot_token');
        $chatId = config('services.telegram.kuongkea_chat_id'); // can be user, group, or channel
    }else{
        $token = config('services.telegram.bot_token');
        $chatId = config('services.telegram.chat_id'); // can be user, group, or channel
...
protected function sendToFacebookPage($imagePath)
{
    ...
    public function ratedisplaytv()
    {
        ...
        $curgold=Currency::where('active',1)->where('ismain',0)->where('iscustomerdisplay',1)->where('isgold',1)->where('company_id',$selcomid)->orderBy('no')->get();
        $cur1=Currency::where('active',1)->where('ismain',0)->where('lmr','l')->where('iscustomerdisplay',1)->where('isgold',0)->where('company_id',$selcomid)->orderBy('no')->get();
        $cur2=Currency::where('active',1)->where('ismain',0)->where('lmr','m')->where('iscustomerdisplay',1)->where('isgold',0)->where('company_id',$selcomid)->orderBy('no')->get();
        $cur3=Currency::where('active',1)->where('ismain',0)->where('lmr','r')->where('iscustomerdisplay',1)->where('isgold',0)->where('company_id',$selcomid)->orderBy('no')->get();
        $curdate=Currency::where('company_id',$selcomid)->max('updated_at');
        $curdate1=ExchangeRate::where('company_id',$selcomid)->max('updated_at');
        $curdate2=ThaiRateSet::where('company_id',$selcomid)->max('updated_at');
        $current = Carbon::now();
        ...
        if($maxdate<$today){
         ...
         if(config('helper.transfer_option') == 'chivra'){
             return view('currencies.ratedisplaytv',compact('cur1','cur2','cur3','curgold','thai_usd','thai_khr','company','maxdate'));
        }else if(config('helper.transfer_option') == 'lpk'){
     ...
     public function getrate_tv()
    {
        ...
        if(config('helper.transfer_option') == 'chivra'){
            ...
            //return view('currencies.spntv',compact('cur1','cur2','cur3','thai_usd','thai_khr','company','maxdate'));
            return view('currencies.spntv_refresh_rate',compact('cur1','cur2','cur3','thai_usd','thai_khr','company','maxdate'));
        }else if(config('helper.transfer_option') == 'kuongkea'){
            return view('currencies.get_refresh_rate_tv',compact('cur1','cur2','cur3','thai_usd','thai_khr','company','maxdate'));
        }else{
             return view('currencies.fullscreenrateboard',compact('cur1','cur2','cur3','thai_usd','thai_khr','company','maxdate'));
    ...
    public function refreshdisplayratetv()
    {
       ...
       $bank_vnd=ThaiRate::where('curname','=','VND')->where('exchangetype','BANK')->where('company_id',$selcomid)->first();
        ...
        return view('currencies.ratedisplaytv1',compact('cur1','cur2','cur3','curgold','thai_usd','thai_khr','maxdate','company'));
    }
    ...
    public function createcurrency(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $c->lmr=$request->lmr;
            $c->allow_user=$request->ck_user_cur;
            $c->imglocation=$request->circleleft;
            $c->lomeang=$request->lomeang;
            $c->isgold=$isgold;
            $c->dostock=$dostock;
            $c->buysale_deposit=str_replace(',','',$request->buysale_deposit);
