 ...
 public function phonenumber_localstorage_thai(Request $request)
{
    ...
    public function quicktransfer(Request $request)
    {
        ...
        $customers=Customer::where('status',1)->where('company_id',$selcomid)->where('customertype','CUSTOMER')->orderBy('no')->get();
        $banks=Customer::where('status',1)->where('company_id',$selcomid)->whereIn('customertype',['BANK'])->orderBy('no')->get();
        $provinces = Address::whereNull('province_id')->get();
    ...
    public function wingtransfer(Request $request)
    {
        ...
        return view('moneytransfers.wingtransfer',compact('partners','partner2s','customers','currencies','users'));

    }
    public function gettransactionname(Request $request)
    {
        $wtn=WingTransactionName::where('agent_type_id',$request->agenttype)->orderBy('num')->get();
        $wtnp=WingTransactionName::where('agent_type_id',$request->agenttype)->where('popular',1)->orderBy('num')->get();
    ...
    public function gettransferlist(Request $request)
    {
      ...
      $selcurtext=$request->selcurtext;
      //$userid=Auth::id();
       $usercapital=UserCapital::whereDate('trandate','=',$dd)->where('agent_id',$request->parrent_id)->where('status',1)->where('trancode',2);
    ...
    //   $transfers=$transfers->where('user_id',$request->user_id);
        ...
        if($request->selcur){
            ...
            $usercapital=$usercapital->where('currency_id',$request->selcur);
        }
        $transfers=$transfers->orderBy('tt')->orderBy('id')->get();
        $usercapital=$usercapital->get();
        if($request->location_id==3){
            return view('moneytransfers.gettransferlistwing',compact('transfers','usercapital','selcurtext'));
        }else if($request->location_id==4){
            return view('moneytransfers.gettransferlistcustomer',compact('transfers','usercapital','selcurtext'));
        }else{
            return view('moneytransfers.gettransferlist',compact('transfers','usercapital','selcurtext'));
        }
    }
    ...
    public function savecashdraw(Request $request)
    {
        ...
        if($request->hasmulticashdraw==1){
          foreach ($request->list_transferid as $key => $value) {
            ...
            if($cashdraw->save()){
              ...
              DB::table('partner_transfers')->where('id',$request->list_transferid[$key])->update(['iscashdraw'=>'1','cashdraw_id'=>$cashdraw_id,'cashdraw_date'=>$opdate1,'ref_number'=>$ref_group_id,'ref_group_id'=>$ref_group_id,'action'=>'']);
          ...
          if($cashdraw->save()){
              ...
              $ref_group_id='cashdraw-' . $cashdraw_id;
              $userprint=Auth::user()->name;
              DB::table('partner_transfers')->where('id',$request->transfer_id)->update(['iscashdraw'=>'1','cashdraw_id'=>$cashdraw_id,'cashdraw_date'=>$opdate,'ref_number'=>'cashdraw-'.$cashdraw_id,'action'=>'']);
              // DB::table('money_print_slips')->insert([
              ...
              //   ]);
                if($request->hascontinue==1){
                    $isbank=$this->checkisbank($request->selpartner_continue);
                    $found_group=1;
                    $ptf1=new PartnerTransfer();
                    ...
                    $ptf1->ref_number=$ref_group_id;
                    $ptf1->ref_group_id=$ref_group_id;
        ...
        if($request->hasexchange==1){
          if(is_null($request->maincur)){
              $this->saveexchangeproduct($request,$opdate,$optime1,$ref_group_id,'បើកវេរប្តូរប្រាក់',$current,Auth::id());
          }else{
              $this->saveexchange($request,$opdate,$optime1,$ref_group_id,'បើកវេរប្តូរប្រាក់',$current,Auth::id());
          }
        }else if($request->hasexchange==2){
            $this->savemultiexchanges($request,$opdate,$optime1,$ref_group_id,'បើកវេរប្តូរប្រាក់',$current,Auth::id());
        }
        if($request->hasexchange>0){
            $found_group=1;
            $es=ExchangeMulti::where('status',1)->where('othercode',$ref_group_id)->get();
            foreach($es as $e){
                if($e->buy>$e->sale){
        ...
        if($request->hasbankpayment==1){
            $found_group=1;
          $this->customerpaytransferbybank($request,$opdate,$optime1,$ref_group_id,'បើកវេរទូទាត់តាមធនាគា',0,$current,'0',Auth::id());
      }
        if($found_group==1){
    ...
    public function delete(Request $request)
    {
        ...
        if($del1->save()){
          if($del1->exchange_list_id){
            ...
            if($del1->thai_list && $del1->trancode==1){
                DB::connection('mysql_thai')->table('sms')->where('transfer_id',$del1->id)->where('accno',$del1->thai_list)->where('sendfrom','partner_transfer')->update(['status'=>0]);
            }
            if(config('helper.hasgoldapp') == '1'){
                DB::table('buy_sale_gold')->where('tran_group','transfer-'.$request->id)->update(['status'=>0,'del_by'=>Auth::id(),'updated_at'=>$current]);
            }
            return response()->json(['success'=>true,'message'=>'this record has been deleted']);
        }
    }
                ...
                if($request->autocashdraw==1){
                    if($request->list_amount[$key]<>0){
                        ...
                        if($cashdraw->save()){
                            $found_group='';
                            $cashdraw_id=$cashdraw->id;
                            DB::table('partner_transfers')->where('id',$tran_id)->update(['iscashdraw'=>'1','cashdraw_id'=>$cashdraw_id,'cashdraw_date'=>$trandate,'ref_number'=>'cashdraw-'.$cashdraw_id,'action'=>'']);
                            //DB::table('cashdraws')->where('id',$cashdraw_id)->update(['ref_group_id'=>'cashdraw-' . $cashdraw_id]);
                        }
                    }
              }
    ...
    public function justupdateinfo(Request $request)
    {
        //return $request->all();
        if ($request->has('id1') && $request->input('id1') > 0) {
            $this->storeupdated($request->id1,0,1);
        }
        $tr=PartnerTransfer::find($request->id1);
        $tr->parrent_id=$request->selpartner;
        $tr->sendername=$request->sendername;
        $tr->sendertel=$request->sendertel;
        $tr->recname=$request->recname;
        $tr->rectel=$request->rectel;
        $tr->save();
    }
    public function store_old(Request $request)
    {
        ...
        //   $data=PartnerTransfer::whereDate('dd',$trandate)->where('user_id',Auth::id())->where('status',1)->orderBy('id')->get()->load('user','partner','currency','cuschargecur','feecurrency');
          ...
          if($ptf->save()){
            ...
            if (!empty($request->thai_list_partner) && $request->thai_list_partner !== 'null') {
                if(config('helper.auto_save_sms_thai_when_transfer') == '1'){
                    $this->savesms($request,$trandate,$trantime,$selcomid,$request->thai_list_partner,$id);
                }
            }
    ...
    public function store(Request $request)
    {
            ...
            if($hasmultitransfer==1){
                ...
                if($ptf->saveOrFail())
                {
                    ...
                    if($request->trancode1==-1){
                        ...
                        if($request->autocashdraw==1){
                            ...
                            if($cashdraw->save()){
                                //$found_group='';
                                    $cashdraw_id=$cashdraw->id;
                                    DB::table('partner_transfers')->where('id',$id)->update(['iscashdraw'=>'1','cashdraw_id'=>$cashdraw_id,'cashdraw_date'=>$trandate,'ref_number'=>'cashdraw-'.$cashdraw_id,'ref_group_id'=>'cashdraw-'.$cashdraw_id,'action'=>'']);
                                    DB::table('cashdraws')->where('id',$cashdraw_id)->update(['ref_group_id'=>'cashdraw-' . $cashdraw_id]);
                                }
                            }
                            if($request->foundcontinue==1){//this case not found in transaction
                    ...
                    if($request->autocashdraw==0){
                     ...
                     if ($request->has('id1') && $request->input('id1') > 0) {
                        DB::connection('mysql_thai')->table('sms')->where('transfer_id',$id)->where('accno',$thai_list)->where('sendfrom','partner_transfer')->update(['amount'=>str_replace(',','',$request->amount)]);
                     }else{
                         if (!empty($request->thai_list_partner) && $request->thai_list_partner !== 'null') {
                             if(config('helper.auto_save_sms_thai_when_transfer') == '1'){
                                 $this->savesms($request,$trandate,$trantime,$selcomid,$request->thai_list_partner,$id);
                    ...
                    //save to buysalegold
                    if(config('helper.hasgoldapp') == '1' && $request->customertype=='CUSTOMER'){
                            $amount_gold=floatval($request->mekun) * floatval(str_replace(',','',$request->amount));
                            $buysale=new BuySaleGold();
                            $buysale->customer_id=$request->selpartner;
                            $buysale->user_id=Auth::id();
                            $buysale->dd=$trandate;
                            $buysale->tt=$trantime;
                            $buysale->currency_id=$request->selcur;
                            $buysale->qty=0;
                            $buysale->rate=0;
                            $buysale->amount=$amount_gold;
                            ...
                            $buysale->pl=0;
                            $buysale->is_ptr=1;
                            $buysale->tran_group=$refnum;
                            $buysale->tran_name=$amount_gold>=0?'ដាក់ប្រាក់':'ដកប្រាក់';
                            $buysale->save();
                    }
                }
            }
     ...
     public function savesms(Request $request,$smsdate,$smstime,$selcomid,$accno,$transfer_id)
    {
        $current = Carbon::now();
        $current->timezone('Asia/Phnom_Penh');
        // $minid = SMS::min('smsid');
        // $smsid=$minid-1;
        $smsid = (SMS::min('smsid') ?? 1) - 1;
        // $minid = SMS::selectRaw('MIN(CAST(smsid AS SIGNED)) as minid')->value('minid');
        // $smsid = ($minid ?? 0) - 1;
        $sms=new SMS();
        $smstext='';
        $sms->company_id=$selcomid;
        $sms->smsid=$smsid;
        $sms->sendfrom='partner_transfer';
        $sms->smsdate=$smsdate;
        $sms->smstime=$smstime;
        $sms->accno=$accno;
        $sms->user_insert=1;
        $sms->transfer_id=$transfer_id;
        $sms->amount=-1 * floatval(str_replace(',','',$request->amount));
        $smstext='cashout from ' . $accno . ' Amount:' . $request->amount . 'B ' . $smsdate . ' ' . $smstime;
        ...
        $sms->isopen=1;
        $sms->smstext=$smstext;
        $sms->smsnote='បង្កើតពេលេវេរតាមដៃគូទៅបញ្ជីថៃ';
