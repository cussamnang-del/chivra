...
class PartnerListController extends Controller
{
    ...
    public function indexnew()
    {
        ...
        if(Auth::user()->role->name=='Admin' || Auth::user()->role->name=='admin'){
            ...
            $userid=Auth::id();
            if(Auth::user()->customer_connect==''){
                $partners=Customer::where('status',1)->whereIn('customertype',['PARTNER','BANK','AGENT','CUSTOMER'])->where('company_id',$selcomid)->orderBy('no')->get();
            }else{
                $partners=Customer::where('status',1)->where('company_id',$selcomid)->whereIn('customertype',['PARTNER','BANK','AGENT','CUSTOMER'])->WhereRaw("FIND_IN_SET(?, user_connect)", [$userid])->orderBy('no')->get();
            }
        }
        $currencies=Currency::where('active',1)->where('company_id',$selcomid)->where('partner_cur',1)->orderBy('no')->get();
        return view('partnerlists.indexnew',compact('partners','currencies'));
    }
    ...
    public function showdata(Request $request)
    {
        ...
        if(isset($request->linkdetail)){
         ...
         if(isset($request->runbalance)){
            $runbalance=$request->runbalance;
        }else{
            $runbalance='false';
        }
        $khr=0;
        ...
        if($request->searchtran==2){
            ...
            if($request->showby=='col'){
                if(isset($request->isprint) && $request->isprint==1){
                    ...
                    return view('partnerlists.reportnew1',compact('theyopen','theyopen_oldlist','theyopen_records','weopen','weopen_oldlist','weopen_records','ptls','ptls1','befortotalwe','befortotalthey','aftertotal','logo','partnername','oldlist','records','ptls_new','linkdetail','runbalance'));
                }
            }else if($request->showby=='cur'){
                if(isset($request->isprint) && $request->isprint==1){
