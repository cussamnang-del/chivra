...
class CaptureController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //     $constraint->upsize();
        // })
        // ->encodeByExtension($ext, quality: 70);
        ...
        if (!$matchedCustomer) {
            ...
            $lastrecord=CustomerExchange::where('company_id',$selcomid)->where('user_id',Auth::id())->whereDate('created_at',$dd)->orderBy('id','desc')->first();
            if($lastrecord){
                $crt = $lastrecord->created_at??''; // UTC
                $rankseconds = $crt->diffInSeconds($tt, false);
                if($rankseconds>=60){//wait 60 second for create new item
                ...
                //start date with the first customer
                $rankseconds=16;
                $matchedCustomer = CustomerExchange::create([
                    'name' => null,
                    'photo' => $path,
            ...
            $lastCaptureCustomer = CustomerExchangeCapture::where('customer_exchange_id', $matchedCustomer->id)->where('company_id',$selcomid)->where('user_id',Auth::id())->whereDate('created_at',$dd)->latest('created_at')->first();
            $crt = $lastCaptureCustomer->created_at??''; // UTC
            $rankseconds = $crt->diffInSeconds($tt, false);

        }
        if($matchedCustomer && $rankseconds>15){
            $lastCapture = CustomerExchangeCapture::where('customer_exchange_id', $matchedCustomer->id)->where('company_id',$selcomid)->where('user_id',Auth::id())->whereDate('created_at',$dd)->latest('created_at')->first();
