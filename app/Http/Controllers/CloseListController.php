...
//get data
  ...
  public function notyetcashdraw(Request $request,$rate_thb,$rate_khr,$rate_vnd)
  {
    ...
    $transfers=PartnerTransfer::select(DB::raw('sum(amount) as total,currency_id'))
            ->where('status',1)->where('trancode',-1)->where('company_id',$selcomid)->whereDate('dd','<=',$d2)
            ->where(function($q) use($d2){
                $q->whereNull('iscashdraw')->orWhereDate('cashdraw_date','>',$d2);
            })
  ...
  public function notyetthaicashdraw(Request $request,$rate_thb)
  {
    ...
    public function summaryallthailist(Request $request,$rate_thb)
    {
        ...
        foreach($thaiaccounts as $c)
        {
            ...
            $allinusd=$thb_usd;
            DB::table('daily_close_lists')->insert(['viewby'=>$viewby,'desr'=>$c->accno,'customer_id'=>$c->id,'modelname'=>'thai_sms','usd'=>$usd,'thb'=>-1 * $thb,'khr'=>$khr,'vnd'=>$vnd,'inusd'=> $allinusd]);
    ...
    public function summarypartnerlist(Request $request)
    {
        ...
        $c=Customer::where('id',$request->cid)->first();
            ...
            //return $closelist;
            //for customer app
             if(config('helper.hasgoldapp') == '1' && $c->customertype=='CUSTOMER'){
                 $deposit_gold=BuySaleGold::where('is_close',0)->where('customer_id',$c->id)->sum('deposit');
                 $pl_gold=BuySaleGold::where('is_close',1)->where('customer_id',$c->id)->where('gold_req',0)->sum('pl');
                 $usd =-1 * $deposit_gold + $pl_gold;
