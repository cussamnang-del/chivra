...
class UserCapitalController extends Controller
{
    ...
    public function seedetail($curid,$curname,$curshortcut,$isexchangecur,$ismain,$viewdate,$userid,$username,$fromdate,$ckcash,$islink,$hide_view,$clearallviewer)
    {
        ...
        if($hide_view==1){
            if($ismain==1){
                ...
                // DB::table('user_report_details')
                // ->whereIn('group_id', function($q) {
                //     $q->select('group_id')
                //     ->from('user_report_details')
                //     ->groupBy('group_id')
                //     ->havingRaw('SUM(amount) = 0')
                //     ->havingNull('agent_name');
                // })
                // ->update(['hide_view' => 1]);
                DB::table('user_report_details')
                ->whereIn('group_id', function ($q) {
                ...
                //     $q->select(DB::raw(1))
                //     ->from('user_report_details as t')
                //     ->whereColumn('t.group_id', 'urd.group_id')
                //     ->whereColumn('t.cur', 'urd.cur')
                //     ->whereNull('t.agent_name')              // <-- FIXED: moved from HAVING to WHERE
                //     ->groupBy('t.group_id', 't.cur')
                //     ->havingRaw('SUM(t.buysale) = 0');       // <-- only aggregate condition stays here
                // })
                // ->update(['hide_view' => 1]);
                ...
                ->whereExists(function ($q) {
                    ...
                    ->whereColumn('x.group_id', 'urd.group_id')
                    ->whereColumn('x.cur', 'urd.cur');
                })
                ->update(['hide_view' => 1]);
    ...
    public function moneyoffer()
    {
        ...
        //$currencies=Currency::where('active',1)->where('ispandp','0')->where('company_id',$selcomid)->where('partner_cur',1)->orderBy('no')->get();
        $currencies=Currency::where('active',1)->where('ispandp','0')->where('company_id',$selcomid)->orderBy('no')->get();
        $user_id=Auth::id();
    ...
    public function getuserbalanceaccount(Request $request)
    {
       ...
       // $sumusercash=DB::table('user_transaction_report_summaries')->where('viewby',Auth::id())->select(DB::raw('currency_id,sum(amount) as tamt'))->groupBy('currency_id')->get();
        ...
        $op=$request->op;
        $customers=Customer::whereIn('id',$customerconnect)->where('status',1)->get();
    ...
    public function showrefgroupid(Request $request)
    {
      ...
      $trid=$group[1];
      $transfer=PartnerTransfer::find($trid);
      ...
      $sms=null;
      if($transfer){
        if($transfer->thai_list){
            $sms=SMS::where('status',1)->where('transfer_id',$trid)->where('accno',$transfer->thai_list)->where('sendfrom','partner_transfer')->get();
        }
      }

      $usercapitals=UserCapital::where('status',1)->where('ref_number',$request->group_id)->orderBy('id')->get();
      if(isset($request->showdelbuton)){
        $showdelbtn=$request->showdelbuton;
      }else{
        $showdelbtn=true;
      }
      return view('usercapitals.grouptransaction',compact('exchanges','usercapitals','exchangemultis','transfers','cashdraws','groupid','partners','currencies','smsprocess','sms','showdelbtn'));
    }
   ...
   public function clear_multiexchangelist(Request $request)
   {
    ...
    public function updatetransactiongroup(Request $request)
    {
      //return $request->all();
        $justeditinfo=0;
        if(isset($request->edit_info) && $request->edit_info==1){
            $justeditinfo=1;
        }
        $transfer1=null;
        $id=$request->id;
        ...
        $cashout=$sumc->where('value','<','0');
        if($transfer){
          return view('usercapitals.updategrouptransaction',compact('transfer','transfer1','transfermultis','transfertemplists','banktransfers','partners','banks','provinces','currencies','mex','totalbuy','totalsale','cashin','cashout','customers','hasbankpayment','hasexchange','hasmultitransfer','justeditinfo'));
    ...
    public function deletetransactiongroup(Request $request)
    {
      //return $request->all();
        ...
        {
            $transfer=PartnerTransfer::find($trid);
            if($transfer && $transfer->trancode==1 && $transfer->thai_list){
                 DB::connection('mysql_thai')->table('sms') ->where('transfer_id',$trid)->where('accno',$transfer->thai_list)->where('sendfrom','partner_transfer')->update(['status' => 0]);
            }
        ...
        if(config('helper.hasgoldapp') == '1'){
            DB::table('buy_sale_gold')->where('tran_group',$groupid)->update(['status'=>0,'del_by'=>Auth::id(),'updated_at'=>$current]);
        }
    ...
    public function gettrueenddingbalanceall_old(Request $request)
    {
        $date = str_replace('/', '-', $request->showdate);
        $trandate = date('Y-m-d', strtotime($date));
        $selcomid=Session('log_into_company_id');
    ...
    public function gettrueenddingbalanceall(Request $request)
    {
        ...
        $endbals = DB::table('user_capitals as uc')
            ...
            // Step 1: collect all currencies and add Total_USD
            $allCurrencies = $endbals->pluck('currency_code')->unique()->values();
            $allCurrencies->push('IN_USD');

            // Step 2: pivot per user
            $pivoted = $endbals->groupBy('user_id')->map(function ($group) use ($allCurrencies) {
            ...
            foreach ($allCurrencies as $currency) {
                ...
                if ($currency === 'IN_USD') {
                    $row['IN_USD'] = number_format($totalUSD, 2, '.', '');
                    continue;
                }
                ...
                // calculate USD converted
                if ($amount != 0) {
                    $totalUSD += $this->exchangetousd($currency, $amount);
                }
            }
    ...
    public function storemoneyofferaccept2(Request $request)
    {
        ...
        try{
                ...
                $uc1->created_at=$current;
                  ...
                  if($request->sellistin<>''){
                    ...
                    $ptf->created_at=$current;
                     ...
                     if($request->customertype2=='BANK' || $request->customertype2=='AGENT'){
                      ...
                      $ptf->note=$note2;
                      $ptf->user_affect=$this->getuseraffectbank($request->sellistin);
                  ...
                  if($request->sellistout<>'')
                  {
                    ...
                    $ptf1->created_at=$current;
                      ...
                      $ptf1->user_affect=$this->getuseraffectbank($request->sellistout);
            ...
            // else{
            //     throw new \Exception("Failed to save UserCapital record.");
            ...
            if($request->amount4 && $request->amount4>0){
                ...
                if($uc->saveOrFail())
                {
                    ...
                    if($request->sellistout1<>''){
                        ...
                        $ptf->ref_number=$refnumber;
                        $ptf->note=$note2;
                        if($request->customertype4=='BANK' || $request->customertype4=='AGENT'){
                            $ptf->isbank=1;
                        }
                        //$ptf->user_affect=$request->usercashin_id;
                        $ptf->user_affect=$this->getuseraffectbank($request->sellistout1);
                        $ptf->updated_at=$current;
                        //$ptf->sendername=$request->noteu2;
                        if($ptf->save()){
                            $lastid=$ptf->id;
                    ...
                    if($request->sellistin1<>'')
                    {
                        ...
                        //$ptf1->user_affect=$request->selusercashin1;
                         if($request->customertype3=='BANK' || $request->customertype3=='AGENT'){
                            $ptf->isbank=1;
                        }
                        $ptf->user_affect=$this->getuseraffectbank($request->sellistin1);
                        //$ptf1->sendername=$request->noteu2;
                        if($ptf1->save()){
                            $idtr2=$ptf1->id;
    ...
    public function store(Request $request)
    {
        ...
        $ref_number='';
        if (!empty($request->agentid) && $request->cashtype=='agent') {
        ...
        {
            if($ref_number<>''){
                $customer=Customer::find($request->agentid);
                if($customer->auto_close_list==1){
         ...
         try{
            ...
            if($uc->saveOrFail())
            {
                ...
                if($request->selbank44<>''){
                    ...
                    $ptf->fee=0;
                    $ptf->fee_currency_id=$request->selcur33;
                    $ptf->bonus=0;
                    // $ptf->sendername=$request->sendername;
                    // $ptf->sendertel=str_replace(' ','',$request->sendertel);
                    // $ptf->recname=$request->txtaccountname44;
                    ...
                    $ptf->user_affect=$this->getuseraffectbank($request->selbank44);
                    $ptf->updated_at=$current;
                    //$ptf->sendername=$request->noteu2;
                    if($ptf->saveOrFail()){
                        $lastid=$ptf->id;
                ...
                if($request->sellist33<>'')
                {
                    ...
                    $ptf1->fee=0;
                    $ptf1->fee_currency_id=$request->selcur33;
                    $ptf1->bonus=0;
                    // $ptf->sendername=$request->sendername;
                    // $ptf->sendertel=str_replace(' ','',$request->sendertel);
                    // $ptf1->recname=$request->txtaccountname44;
                    ...
                    $ptf1->user_affect=$this->getuseraffectbank($request->sellist33);
                    //$ptf1->sendername=$request->noteu2;
                    if($ptf1->saveOrFail()){
                        $idtr2=$ptf1->id;
                        //DB::table('user_capitals')->where('id',$id)->update(['ref_number'=>$id1]);
