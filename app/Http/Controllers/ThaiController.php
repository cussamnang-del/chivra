    ...
    public function thaiaccountcloselist(Request $request)
    {
     ...
     public function getaccountlistreport(Request $request){
        ...
        if($request->oldlist==='true'){
            ...
            $oldlist=Sms::where('accno',$request->accno)->where('status',1)->where('id','>',$lastsmsid)->whereDate('smsdate','>=',$closedate)->whereDate('smsdate','<',$d1)->whereNull('mix_from_id')->sum('amount');

        }

        $data=Sms::where('accno',$request->accno)->where('status',1)->whereBetween(DB::raw('DATE(smsdate)'), array($d1, $d2))->where('id','>',$lastsmsid)->whereNull('mix_from_id');
        if($request->customer){
