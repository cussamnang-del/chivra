...
class LoginController extends Controller
{
     ...
     public function getuserbycompany(Request $request){
        ...
        $users = User::where('active', 1)->where('is_activated',1)->where('role_id','>',0)
     ...
     public function showlogin()
     {
        ...
        if(!$comid){
            $comid=Company::orderBy('id')->first()->value('id');
        }
        $users = User::where('active', 1)->where('is_activated',1)->where('role_id','>',0)
            ...
            if($U->role->name=='Admin'){
              ...
              if (Auth()->guard('web')->attempt($attemp)) {
                ...
                if(in_array($get_internet_ip, $public_ip)){
                    ...
                    if(Hash::check($request->password1, $U->remote_password)){
                        DB::table('users')->where('id', $U->id)->update(['session_id' => $new_sessid,'attempt'=>0]);
                        return redirect($this->redirectTo);
                    }else{
    ...
    public function create()
    {
        ...
        $users = User::where('active', 1)->where('role_id','>',0)
        ->where(function ($q) use ($selcomid) {
            $q->where('company_id', $selcomid)
            ->orWhere('company_id', '')
            ->orWhereNull('company_id');
        ...
        $companies=Company::where('status',1)->get();
        $customers=Customer::where('status',1)->where('company_id',$selcomid)->whereIn('customertype',['BANK','AGENT','CUSTOMER'])->orderBy('customertype')->get();
    ...
    public function refreshuser(Request $request)
    {
        ...
        if ($comid == '') {
            if($request->radact==1){
                $users = User::where('active', $request->radact)->where('role_id','>',0)->orderBy('id')->get();
            }else if($request->radact==2){
                 $users = User::where('active', 1)->where('role_id','=',0)->orderBy('id')->get();
            }else if($request->radact==0){
                 $users = User::where('active', $request->radact)->orderBy('id')->get();
            }

        } else {
            if($request->radact==1){
                $users = User::where('active', $request->radact)->where('role_id','>',0)
                ->where(function ($q) use ($comid) {
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user = User::create([
              ...
              'connect_customer_id' => $request->sellistwith,
  ...
  protected function registered(Request $request, $user)
  {
    ...
    {
      if($request->col=='active'){
          DB::table('users')->where('id',$request->id)->update(['active'=>!$request->active]);
      }else if($request->col=='use_camra'){
         DB::table('users')->where('id',$request->id)->update(['use_camra'=>!$request->active]);
      }else if($request->col=='activated'){
         DB::table('users')->where('id',$request->id)->update(['is_activated'=>!$request->active]);
      }
     ...
     public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user->connect_customer_id=$request->sellistwith;
    ...
    {
      $users=User::where('id','<>',$request->uid)->where('role_id','<>',1)->where('company_id',$request->company)->get();
