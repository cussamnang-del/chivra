...
class RealEstateController extends Controller
{
   ...
   public function getpaidcommission(Request $request)
    ...
    public function getcommissionlistall(Request $request)
    {
        ...
        //     $transfers=PartnerTransfer::where('status',1)->where('location_id',8)->where('isbank','<>',1)
        //     ->where(function($q) use($d1,$d2){
        ...
        // }else if($request->seldata==-1){ //មិនទាន់ទូទាត់ + ខែចាស់
        //     $transfers=PartnerTransfer::where('status',1)->where('location_id',8)->where('isbank','<>',1)
        //     ->where(function($q) use($d1,$d2){
        //         $q->where(function($q1) use($d1,$d2){
        //              $q1->whereBetween(DB::raw('DATE(payformonth)'), array($d1, $d2))->where('amount',0)->whereIn('trancode',[-1,-4])->whereNull('ispaytosaler');
        ...
        // }else{
        //     $transfers=PartnerTransfer::where('status',1)->where('location_id',8)->where('isbank','<>',1)->whereBetween(DB::raw('DATE(payformonth)'), array($d1, $d2))->whereIn('trancode',[-1,-4])
        //     ->where(function($q){
        ...
        if($request->seldata==0){ //មិនទាន់ទូទាត់
            ...
            ->where(function($q) use($m,$y){
                $q->where(function($q1) use($m,$y){
                     $q1->whereMonth('payformonth',$m)->whereYear('payformonth', $y)->where('amount',0)->whereIn('trancode',[-1,-4])->whereNull('ispaytosaler');
                })->orWhere(function($q2) use($m,$y){
                    //$q2->whereMonth('payformonth','<',$m)->whereYear('payformonth', $y)->where('amount',0)->whereIn('trancode',[-1,-4])->whereNull('ispaytosaler');
                })->orWhere(function($q3) use($m,$y){
                    $q3->whereMonth('created_at',$m)->whereYear('created_at', $y)->where('amount',0)->whereIn('trancode',[-1,-4])->whereNull('ispaytosaler');
                });
            });
            ...
            $transfers=PartnerTransfer::where('status',1)->where('location_id',8)->where('isbank','<>',1)
             ->where(function($q) use($m,$y){
                $q->where(function($q1) use($m,$y){
                     $q1->whereMonth('payformonth',$m)->whereYear('payformonth', $y)->where('amount','<>',0)->whereIn('trancode',[-1,-4])->where('ispaytosaler',1);
                })->orWhere(function($q3) use($m,$y){
                    $q3->whereMonth('created_at',$m)->whereYear('created_at', $y)->where('amount','<>',0)->whereIn('trancode',[-1,-4])->where('ispaytosaler',1);
                });
            });

        }else{
            $transfers=PartnerTransfer::where('status',1)->where('location_id',8)->where('isbank','<>',1)->whereMonth('payformonth',$m)->whereYear('payformonth', $y)->whereIn('trancode',[-1,-4])
            ->where(function($q){
                $q->where('amount',0)->orWhere('ispaytosaler',1);
            });
        ...
        if(isset($request->isprint)){
            ...
            return view('realestates.getcommissionlistprintall',compact('transfers','rpttitle','d1','d2','printdatetext'));
        }else{
    ...
    public function commissionlistall(Request $request)
    {
        ...
        return view('realestates.commissionlistall',compact('transfers','partners','currencies','salers','allproperty','groups','month','year'));
    }
    ...
    public function searchcloselist(Request $request)
    {
        ...
        foreach($transfers as $t){
            ...
            $end   = Carbon::create($y, $m, 1)->endOfMonth();
            $payover = PartnerTransfer::where([
                ['status', '=', 1],
                ['id', '<>', $t->payonid],
                ['payonid', '=', $t->payonid],
            ])
            // dd must be in selected month/year
            ->whereBetween('dd', [$start, $end])
            ->whereNotBetween('payformonth', [$start, $end])
            ->where('trancode', '<>', -8)
            ->whereNotNull('nextpayment')

            // payformonth must NOT be in selected month/year
            // ->where(function ($q) use ($start, $end) {
            //     $q->whereDate('payformonth', '<', $start)
            //     ->orWhereDate('payformonth', '>', $end);
            // })
            ...
            ->selectRaw('
                ...
                SUM(cuscharge) as total_cuscharge,
                SUM(cuscharge_debt) as total_cuscharge_debt,
    ...
    public function getsoldlist(Request $request)
    {
        ...
        $selgroup=$request->selgroup;
        $loancomplete=0;
        if($request->loancomplete=='true'){
            $loancomplete=1;
        }
        if($request->selgroup=='all' || $request->property_id>0){
            if($request->property_id){
                $saleid=SaleDetail::where('status',1)->where('property_id',$request->property_id)->pluck('sale_id')->toArray();
                $transfers=PartnerTransfer::where('status',1)->whereIn('id',$saleid)->where('loancomplete',$loancomplete);
            }else{
                $transfers=PartnerTransfer::where('status',1)->where('trancode',$request->trancode)->where('loancomplete',$loancomplete);
            }
            ...
            $transfers = DB::table('partner_transfers')
                ...
                ->where('partner_transfers.status', 1)
                ->where('partner_transfers.loancomplete', $loancomplete)
                ...
                ->select(
                    ...
                    DB::raw('MAX(partner_transfers.loancomplete) as loancomplete'),
    ...
    public function depositprint(Request $request){
       ...
       $id=$request->id;
        ...
        if($transfer){
            ...
            $transfergroup=PartnerTransfer::where('ref_group_id',$request->groupid)->whereNotNull('ref_group_id')->whereNotIn('trancode',[-8,8])
            ->where('parrent_id',$transfer->parrent_id)
            ->where('status',1)
            ->where('tranname','<>','Discount')
    ...
    public function deletepaidcommissionall(Request $request)
{
    ...
    public function savedeposit(Request $request)
    {
        ...
        if($checktrid){
            ...
            for($i=0;$i<$qty;$i++){  //ភ្ញៀវចង់បង់ច្រើនជាង១ខែ
                ...
                $ptf->fee_ex=0;
                $ptf->fee_currency_id=$request->curid;
                if($i==0 && isset($request->bonus) && $request->bonus>0){
                    $ptf->bonus=str_replace(',','',$request->bonus);
                }else{
                    $ptf->bonus=0;
                }
                ...
                if($ptf->save()){
                    ...
                    if($i==0){
                        ...
                        if(isset($request->bonus) && $request->bonus>0){
                            $ptf5=new PartnerTransfer();
                            $ptf5->tranname='Discount';
                            $ptf5->trancode=1;
                            $ptf5->mekun=1;
                            $ptf5->dd=$trandate;
                            $ptf5->tt=$trantime;
                            $ptf5->user_id=Auth::id();
                            $ptf5->parrent_id=$request->partner_id;
                            $ptf5->amount=str_replace(',','',$request->bonus);
                            $ptf5->currency_id=$request->curid;
                            $ptf5->cuscharge=0;
                            $ptf5->cuscharge_ex=0;
                            $ptf5->cuscharge_currency_id=$request->curid;
                            $ptf5->fee=0;
                            $ptf5->fee_ex=0;
                            $ptf5->fee_currency_id=$request->curid;
                            $ptf5->bonus=0;
                            $ptf5->sendername=$request->property;
                            $ptf5->property_group=$request->property_group;
                            $ptf5->property_id=$request->property_id;
                            $ptf5->payonid=$request->id1;
                            $ptf5->note=$request->note;
                            $ptf5->location_id=8;
                            if(isset($request->isromlos)){
                                $ptf5->payformonth=date('Y-m-d', strtotime( "+ $i months", strtotime($request->payformonth)));
                                $ptf5->nextpayment=date('Y-m-d', strtotime( "+ $j months", strtotime($request->payformonth)));
                            }
                            $ptf5->ref_group_id=$refgroupid;
                            $ptf5->created_at=$current;
                            $ptf5->updated_at=$current;
                            if($ptf5->save()){
    ...
    public function payment(Request $request)
    {
        ...
        $cusdebt=PartnerTransfer::where('payonid',$request->id)->where('status',1)->where('tranname','<>','Discount')->orderBy('id','desc')->first();
    ...
    public function showromloslist(Request $request)
    {
        ...
        ->orderBy('id')->get();
        $myc= collect();
        $lastromlos=PartnerTransfer::where('payonid',$request->id)->where('id','<>',$request->id)->where('parrent_id',$cid)->where('status',1)->where('tranname','<>','Discount')->orderBy('id','desc')->first();
        foreach($transfers as $t){
            if($lastromlos){
        ...
        for($i=0;$i<$term;$i++){
            ...
            $found=PartnerTransfer::where('payonid',$request->id)->where('parrent_id',$cid)->whereNotNull('payformonth')->whereMonth('payformonth',$month)->whereYear('payformonth',$year)->where('status',1)->where('tranname','<>','Discount')->orderBy('id','desc')->first();
            ...
            if($found){
                ...
                $myc=$myc->push(['id'=>$found->id,'payonid'=>$request->id,'dd'=>$found->payformonth,'payformonth'=>$found->payformonth,'tt'=>$found->tt,'usersave'=>$found->user->name,'trancode'=>$found->trancode,'tranname'=>$found->tranname,'amount'=>$found->amount + $found->bonus,'curid'=>$found->currency_id,'currency'=>$found->currency->sk,'curname'=>$found->currency->shortcut,'sendername'=>$found->sendername,'groupid'=>$found->ref_group_id,'trandate'=>$found->dd,'action'=>$action]);
    ...
    public function updatetransactiondate(Request $request)
    {
        $request->validate([
        'id' => 'required|integer',
        'dd' => 'required|date_format:d-m-Y'
    ]);

    $date = \Carbon\Carbon::createFromFormat('d-m-Y', $request->dd)
            ->format('Y-m-d');

    \App\PartnerTransfer::where('id', $request->id)
        ->update(['dd' => $date]);
    ...
    public function viewallcustomertransaction(Request $request)
    {
        //return $request->all();
        $property=$request->sendername;
        $customername=$request->buyer;
        $lists=PartnerTransfer::where('parrent_id',$request->customer_id)->where('payonid',$request->id)->where('status',1)->orderBy('id')->get();
        return view('realestates.viewcustomertransaction',compact('lists','property','customername'));
    }
    ...
    public function searchromlos(Request $request)
    {
        ...
        for($i=0;$i<$term;$i++){
            ...
            if($found){
                ...
                $myc=$myc->push(['id'=>$found->id,'payonid'=>$request->id,'dd'=>$found->payformonth,'payformonth'=>$found->payformonth,'tt'=>$found->tt,'usersave'=>$found->user->name,'trancode'=>$found->trancode,'tranname'=>$found->tranname,'amount'=>$found->amount + $found->bonus,'curid'=>$t->currency_id,'currency'=>$found->currency->sk,'curname'=>$found->currency->shortcut,'sendername'=>$found->sendername,'groupid'=>$found->ref_group_id,'trandate'=>$found->dd,'action'=>$action]);
    ...
    public function buypaymentcompleted(Request $request)
    {
        $current = Carbon::now();
        $current->timezone('Asia/Phnom_Penh');
        $loan=PartnerTransfer::find($request->id);
        if($loan){
            if(DB::table('partner_transfers')->where('id',$request->id)->update(['loancomplete'=>!$loan->loancomplete,'updated_at'=>$current])){
                DB::table('partner_transfers')->where('payonid',$request->id)->update(['loancomplete'=>!$loan->loancomplete,'updated_at'=>$current]);
    ...
    public function deletepayment(Request $request)
    {
     ...
     public function deletecomzero(Request $request)
    {
        $tr=PartnerTransfer::find($request->id);
        $tr->status=0;
        $tr->user_delete=Auth::id();

        if($tr->save()){
            return response()->json(['success'=>true,'message'=>'this transaction have been remove']);

        }else{
            return response()->json(['error'=>true,'message'=>'delete transaction fail']);
        }
